...
use ghost_shell_gpui::{
    BoxShadow, FontWeight, Hsla, Pixels, SharedString, hsla, point, px, rgb,
};
...
mod tests {
    ...
    #[test]
    fn older_palettes_receive_a_selection_color() -> Result<(), serde_json::Error> {
        let mut palette = serde_json::to_value(ColorTokens::dark())?;
        let expected: ColorTokens = serde_json::from_value(palette.clone())?;
        if let Some(fields) = palette.as_object_mut() {
            fields.remove("selection");
        }
        let restored: ColorTokens = serde_json::from_value(palette)?;
        assert_eq!(restored.selection, ColorTokens::light().selection);
        assert_eq!(restored.background, expected.background);
        Ok(())
    }
