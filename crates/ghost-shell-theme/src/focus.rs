...
pub fn init(cx: &mut App) {
    if !cx.has_global::<FocusTrapManager>() {
...
impl FocusTrapManager {
    ...
    fn register_trap(
        id: &GlobalElementId,
        container_handle: WeakFocusHandle,
        cx: &mut App,
    ) {
    ...
    fn find_active_trap(window: &Window, cx: &App) -> Option<FocusHandle> {
        let mut active: Option<FocusHandle> = None;
        for (_id, container_handle) in Self::global(cx).traps.iter() {
            ...
            if container.contains_focused(window, cx)
                && active
                    .as_ref()
                    .is_none_or(|outer| outer.contains(&container, window))
            {
...
impl<E: InteractiveElement + ParentElement + Styled + Element> Styled
    for FocusTrapContainer<E>
{
    ...
    for FocusTrapContainer<E>
{
    ...
    fn request_layout(
        &mut self,
        global_id: Option<&ghost_shell_gpui::GlobalElementId>,
        ...
        if let Some(global_id) = global_id {
            init(cx);
            FocusTrapManager::register_trap(global_id, self.focus_handle.downgrade(), cx);
        }
...
pub fn active_focus_trap(window: &Window, cx: &App) -> Option<FocusHandle> {
    cx.try_global::<FocusTrapManager>()?;
