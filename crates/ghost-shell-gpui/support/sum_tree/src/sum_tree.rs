    ...
    D3: Dimension<'a, S>,
{
    fn cmp(
        &self,
        cursor_location: &Dimensions<D1, D2, D3>,
        cx: S::Context<'_>,
    ) -> Ordering {
...
impl<T: Item> SumTree<T> {
    ...
    pub fn from_iter<I: IntoIterator<Item = T>>(
        ...
        while iter.peek().is_some() {
            ...
            let item_summaries: ArrayVec<T::Summary, { 2 * TREE_BASE }, u8> = items
                .iter()
                .map(|item| item.summary(cx))
                .collect();
        ...
        while nodes.len() > 1 {
            ...
            nodes = nodes
                ...
                .map(|child_nodes| {
                    ...
                    let child_summaries: ArrayVec<T::Summary, { 2 * TREE_BASE }, u8> =
                        child_trees
                            .iter()
        ...
        'iterate: loop {
            match &*this.0 {
                Node::Internal {
                    ...
                    for (child_tree, child_summary) in
                        child_trees.iter().zip(child_summaries)
                    {
                        let child_end = position
                            .clone()
                            .with_added_summary(child_summary, cx);
        ...
        Target: SeekTarget<'slf, T::Summary, D>,
    {
        ...
        let mut pos = D::zero(cx);
        return match Self::find_with_prev_iterate::<_, _, false>(
            cx, target, bias, &mut pos, self,
        ) {
    ...
    fn update_last_recursive(
        ...
        match Arc::make_mut(&mut self.0) {
            ...
            Node::Leaf {
                ...
                if let Some((item, item_summary)) = items
                    .last_mut()
                    .zip(item_summaries.last_mut())
                {
                    (f)(item);
        ...
        match Arc::make_mut(&mut self.0) {
            Node::Internal {
                ...
                let mut summaries_to_append =
                    ArrayVec::<T::Summary, { 2 * TREE_BASE }, u8>::new();
                let mut trees_to_append =
                    ArrayVec::<SumTree<T>, { 2 * TREE_BASE }, u8>::new();
                if height_delta == 0 {
                    summaries_to_append
                ...
                if child_count > 2 * TREE_BASE {
                    ...
                    let midpoint = (child_count + child_count % 2) / 2;
                    {
                        let mut all_summaries = child_summaries
                            ...
                            .cloned();
                        left_summaries = all_summaries
                            .by_ref()
                            .take(midpoint)
                            .collect();
                        right_summaries = all_summaries.collect();
                        let mut all_trees = child_trees
                            .iter()
                            .chain(trees_to_append.iter())
                            .cloned();
            ...
            (
                ...
                if total_child_count <= 2 * TREE_BASE {
                    ...
                    *summary = full_summary;
                    ...
                    let mut all_trees = small_child_trees
                        .iter()
                        .chain(child_trees.iter())
                        .cloned();
    ...
    fn from_child_trees(
        ...
        child_summaries
            .push(left.0.summary().clone())
            .unwrap_oob();
        child_summaries
            .push(right.0.summary().clone())
            .unwrap_oob();
...
mod tests {
    ...
    fn test_random() {
        ...
        let num_operations = std::env::var("OPERATIONS").map_or(5, |o| {
            o.parse()
                .expect("invalid OPERATIONS variable")
        });
            ...
            for _ in 0..num_operations {
                ...
                while item_ix < expected_filtered_items.len() {
                    ...
                    let (reference_index, reference_item) =
                        expected_filtered_items[item_ix];
    ...
    fn test_edit() {
        ...
        let removed =
            tree.edit(vec![Edit::Insert(2), Edit::Insert(4), Edit::Remove(0)], ());
