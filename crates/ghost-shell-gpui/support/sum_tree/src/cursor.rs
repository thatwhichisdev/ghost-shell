    ...
    D: Dimension<'a, T::Summary>,
{
        ...
        F: FnMut(&T::Summary) -> bool,
    {
                ...
                match entry.tree.0.as_ref() {
                    Node::Internal {
                        ...
                        while entry.index() < child_summaries.len() {
                            ...
                            if filter_node(next_summary) {
                                ...
                                entry
                                    .position
                                    .add_summary(next_summary, self.cx);
                                self.position
                                    .add_summary(next_summary, self.cx);
                            }
                        }
                    ...
                    Node::Leaf { item_summaries, .. } => {
                        if !descend {
                            ...
                            entry
                                .position
                                .add_summary(item_summary, self.cx);
                            self.position
                                .add_summary(item_summary, self.cx);
                        }

                        loop {
                            if let Some(next_item_summary) =
                                item_summaries.get(entry.index())
                            {
                                if filter_node(next_item_summary) {
                                    ...
                                    entry
                                        .position
                                        .add_summary(next_item_summary, self.cx);
                                    self.position
                                        .add_summary(next_item_summary, self.cx);
                                }
                            } else {
        ...
        debug_assert!(
            self.stack.is_empty() || self.stack.last().unwrap().tree.0.is_leaf()
        );
    }
    ...
    D: Dimension<'a, T::Summary>,
{
    ...
    pub fn next(&mut self) {
        self.cursor
            .search_forward(&mut self.filter_node);
    }

    pub fn prev(&mut self) {
        self.cursor
            .search_backward(&mut self.filter_node);
    }
}
