...
/// makes it safe to call from threads that must not block, such as the UI thread.
///
    ...
    Fut::Output: Send + 'static,
{
    ...
    let thread_body = move || {
        let (runnable_sender, runnable_receiver) =
            flume::unbounded::<Runnable<RunnableMeta>>();
