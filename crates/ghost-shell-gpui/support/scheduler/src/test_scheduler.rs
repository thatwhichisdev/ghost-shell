...
impl TestScheduler {
        ...
        (seed..seed + num_iterations as u64)
            .map(|seed| {
                ...
                match std::panic::catch_unwind(move || {
                    Self::with_seed(seed, &mut *unwind_safe_f)
                }) {
                    Ok(result) => result,
                    Err(error) => {
    ...
    fn with_seed<R>(seed: u64, f: impl AsyncFnOnce(Arc<TestScheduler>) -> R) -> R {
        let scheduler =
            Arc::new(TestScheduler::new(TestSchedulerConfig::with_seed(seed)));
        ...
        let result = output
            .take()
            .expect("test future did not complete");
    ...
    fn block_until(
        ...
        let waker =
            unsafe { Waker::new(Box::into_raw(waker) as *const (), &WAKER_VTABLE) };
            ...
            if candidate_indices.is_empty() {
                ...
                if total_weight == 0 {
                    ...
                    let choice = self
                        .rng
                        .lock()
                        .random_range(0..candidate_indices.len());
                    state
                        .runnables
                    ...
                    state
                        .runnables
                        .remove(candidate_indices[selected_idx])
                }
            } else {
    ...
    pub fn advance_clock_to_next_timer(&self) -> bool {
        if let Some(timer) = self.state.lock().timers.first() {
            self.clock
                .advance(timer.expiration - self.clock.now());
            true
        } else {
            false
        }
    ...
    pub fn advance_clock(&self, duration: Duration) {
        ...
        loop {
                ...
                && timer.expiration <= next_now
            {
                ...
                self.clock
                    .advance(advance_to - self.clock.now());
            } else {
                break;
            }
        }
        self.clock
            .advance(next_now - self.clock.now());
        if debug {
            eprintln!(
...
impl Scheduler for TestScheduler {
    ...
    fn schedule_background_with_priority(
        ...
        let ix = if state.randomize_order {
            self.rng
                .lock()
                .random_range(0..=state.runnables.len())
        } else {
            state.runnables.len()
        };
        state.runnables.insert(
    ...
    fn timer(&self, duration: Duration) -> Timer {
        ...
        state
            .timers
            .sort_by_key(|timer| timer.expiration);
        Timer(rx)
    }
    ...
    fn spawn_dedicated(
        ...
        f: Box<
            dyn FnOnce(
                    ...
                    Box<dyn Future<Output = Box<dyn Any + Send + Sync>> + 'static>,
                > + Send
                + 'static,
        >,
...
impl Clone for TracingWaker {
    fn clone(&self) -> Self {
        ...
        let id = if state.capture_pending_traces {
            ...
            state
                .pending_traces
                .insert(id, Backtrace::new_unresolved());
            Some(id)
        } else {
            None
        };
    ...
    let waker_clone_frame_ix = frames.iter().position(|frame| {
        frame.symbols().iter().any(|symbol| {
            symbol.name().is_some_and(|name| {
                format!("{name:#?}") == type_name_of_val(&Waker::clone)
            })
        })
    });
