...
impl LocalExecutor {
    ...
    pub fn block_on<Fut: Future>(&self, future: Fut) -> Fut::Output {
        ...
        output
            .take()
            .expect("block_on future did not complete")
    }
...
impl<T> RendezvousReceiver<T> {
    fn poll_take(&self, cx: &mut Context) -> Poll<Task<T>> {
        ...
        match std::mem::replace(&mut *state, RendezvousState::Taken) {
            ...
            | RendezvousState::Taken => {
                unreachable!(
                    "a rendezvous task was polled after its receiver was consumed"
                )
            }
        }
    }
    ...
    fn detach(self) {
        ...
        match std::mem::replace(&mut *state, RendezvousState::Detached) {
            ...
            | RendezvousState::Taken => {
                unreachable!(
                    "a rendezvous task was detached after its receiver was consumed"
                )
            }
        }
    }
    ...
    fn drop(&mut self) {
        ...
        match &*state {
            ...
            RendezvousState::Delivered(_) => {
                let delivered =
                    std::mem::replace(&mut *state, RendezvousState::Cancelled);
                drop(state);
                drop(delivered);
            }
...
impl<T> std::fmt::Debug for Task<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.0 {
            ...
            TaskState::Spawned(task) => f
                .debug_tuple("Task::Spawned")
                .field(task)
                .finish(),
            TaskState::Downcast { inner, .. } => f
                .debug_tuple("Task::Downcast")
                .field(inner)
                .finish(),
...
impl<T> std::fmt::Debug for FallibleTask<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match &self.0 {
            ...
            FallibleTaskState::Spawned(task) => f
                .debug_tuple("FallibleTask::Spawned")
                .field(task)
                .finish(),
            FallibleTaskState::Downcast { inner, .. } => f
                ...
                .field(inner)
                .finish(),
            FallibleTaskState::Rendezvous(_) => f
                .debug_tuple("FallibleTask::Rendezvous")
                .finish(),
        }
    }
}
