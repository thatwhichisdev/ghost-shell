...
/// ```
#[derive(
    Refineable, Copy, Clone, Default, Debug, Eq, PartialEq, Serialize, Deserialize, Hash,
)]
#[refineable(Debug)]
#[repr(C)]
...
impl<T: Clone + Debug + Default + PartialEq + Display + Add<T, Output = T>> Display
    for Bounds<T>
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
...
impl Edges<DefiniteLength> {
    ...
    pub fn to_pixels(
        ...
        Edges {
            top: self
                .top
                .to_pixels(parent_size.height, rem_size),
            right: self
                .right
                .to_pixels(parent_size.width, rem_size),
            bottom: self
                .bottom
                .to_pixels(parent_size.height, rem_size),
            left: self
    ...
    T: Add<T, Output = T> + Half + Clone + Debug + Default + PartialEq,
{
    ...
    pub fn corner(&self, corner: Anchor) -> T {
        match corner {
            ...
            Anchor::LeftCenter => {
                (self.top_left.clone() + self.bottom_left.clone()).half()
            }
            Anchor::RightCenter => {
                (self.top_right.clone() + self.bottom_right.clone()).half()
            }
        }
    }
}
...
/// display resolutions.
#[derive(
    Clone, Copy, Default, Add, AddAssign, Sub, SubAssign, Div, DivAssign, PartialEq,
)]
#[repr(transparent)]
