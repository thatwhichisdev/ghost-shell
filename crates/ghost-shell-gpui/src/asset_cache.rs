...
mod tests {
    ...
    #[gpui::test]
    fn completed_load_without_subscribers_is_cached_and_deduplicated(
    ...
    fn load_completion_notifies_each_observing_entity_once(cx: &mut TestAppContext) {
        ...
        let (first_entity, second_entity) = cx.update(|cx| {
            ...
            cx.observe(&first_entity, {
                ...
                move |_, _| {
                    first_notification_count.set(first_notification_count.get() + 1)
                }
            })
            .detach();
            cx.observe(&second_entity, {
                ...
                move |_, _| {
                    second_notification_count.set(second_notification_count.get() + 1)
                }
            })
            .detach();
    ...
    #[gpui::test]
    fn eviction_after_worker_completion_does_not_publish_into_replacement(
        cx: &mut TestAppContext,
    ) {
        let (sender, receiver) = oneshot::channel();
        let (replacement_sender, replacement_receiver) = oneshot::channel();
