use std::{
    cell::RefCell, future::Future, ops::Deref, path::PathBuf, rc::Rc, sync::Arc,
    time::Duration,
};
...
use crate::{
    ...
    BackgroundExecutor, BorrowAppContext, Bounds, Capslock, ClipboardItem, DrawPhase,
    Drawable, Element, Empty, EntityId, EventEmitter, ForegroundExecutor, Global,
    InputEvent, Keystroke, Modifiers, ModifiersChangedEvent, MouseButton, MouseDownEvent,
    MouseMoveEvent, MouseUpEvent, Pixels, Platform, PlatformTextSystem, Point, Render,
    Result, SharedString, Size, SystemNotification, SystemNotificationResponse, Task,
    TestDispatcher, TestPlatform, TestWindow, TextSystem, VisualContext, Window,
...
impl TestAppContext {
    ...
    pub fn set_idle_sleep_prevention_delay(&self, delay: Duration) {
        self.test_platform
            .set_idle_sleep_prevention_delay(delay);
    }
    ...
    pub fn set_idle_sleep_prevention_fails(&self, fails: bool) {
        self.test_platform
            .set_idle_sleep_prevention_fails(fails);
    }
    ...
    pub fn next_notification(
        ...
        async move {
            rx.recv().await.expect(
                "entity dropped while test was waiting for its next notification",
            );
            drop(subscription);
        }
    }
...
impl VisualTestContext {
    ...
    pub fn simulate_keystrokes(&mut self, keystrokes: &str) {
        self.cx
            .simulate_keystrokes(self.window, keystrokes)
    }
...
mod tests {
    ...
    #[gpui::test]
    async fn test_system_notification_response_handler_can_be_replaced(
        cx: &mut TestAppContext,
    ) {
    ...
    #[gpui::test]
    async fn test_system_notification_response_handler_can_reenter_app(
        cx: &mut TestAppContext,
    ) {
        cx.update(|cx| {
            cx.set_app_identity("com.example.tasks", "Tasks");
