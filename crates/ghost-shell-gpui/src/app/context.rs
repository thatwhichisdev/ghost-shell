...
use crate::{
    AnyView, AnyWindowHandle, AppContext, AsyncApp, DispatchPhase, Effect, EntityId,
    EventEmitter, FocusHandle, FocusOutEvent, Focusable, Global, KeystrokeObserver,
    Priority, Reservation, SubscriberSet, Subscription, Task, WeakEntity,
    WeakFocusHandle, Window, WindowHandle, WindowVisibility,
};
...
impl<'a, T: 'static> Context<'a, T> {
        ...
        W: 'static,
    {
        ...
        self.app
            .observe_internal(entity, move |entity, cx| {
        ...
        Evt: 'static,
    {
        let this = self.entity();
        self.app
            .subscribe(&this, move |this, evt, cx| {
        ...
        T: 'static,
    {
        ...
        self.app.on_app_restart(move |cx| {
            handle
                .update(cx, |entity, cx| on_restart(entity, cx))
                .ok();
        })
    }
        ...
        T: 'static,
    {
        ...
        self.app.on_app_quit(move |cx| {
            let future = handle
                .update(cx, |entity, cx| on_quit(entity, cx))
                .ok();
            async move {
        ...
        R: 'static,
    {
        let this = self.weak_entity();
        self.app
            .spawn(async move |cx| f(this, cx).await)
    }
        ...
        T: 'static,
    {
        ...
        self.new_observer(
            observed_id,
            Box::new(move |cx| {
                invoke_observer_in(&observer, &observed, cx, &mut on_notify)
            }),
        )
    }
    ...
    pub fn subscribe_in<Emitter, Evt>(
        ...
        window: &Window,
        mut on_event: impl FnMut(&mut T, &Entity<Emitter>, &Evt, &mut Window, &mut Context<T>)
        + 'static,
    ) -> Subscription
    where
    ...
    pub fn on_release_in(
        ...
        self.app
            .observe_release_in(&entity, window, on_release)
    }
    ...
    pub fn observe_window_visibility(
        &self,
        window: &mut Window,
        mut callback: impl FnMut(&mut T, WindowVisibility, &mut Window, &mut Context<T>)
        + 'static,
    ) -> Subscription {
        let view = self.weak_entity();
    ...
    pub fn on_focus_out(
        ...
        handle: &FocusHandle,
        window: &mut Window,
        mut listener: impl FnMut(&mut T, FocusOutEvent, &mut Window, &mut Context<T>)
        + 'static,
    ) -> Subscription {
        let view = self.weak_entity();
        let focus_id = handle.id;
    ...
    pub fn on_action(
        ...
        action_type: TypeId,
        window: &mut Window,
        listener: impl Fn(&mut T, &dyn Any, DispatchPhase, &mut Window, &mut Context<T>)
        + 'static,
    ) {
        let handle = self.weak_entity();
