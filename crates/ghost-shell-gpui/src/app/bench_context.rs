...
use crate::{
    ...
    PlatformHeadlessRenderer, PlatformTextSystem, Render, Reservation, Task,
...
impl BenchReport {
    ...
    fn record_foreground_events<'i>(
        &self,
        events: impl IntoIterator<Item = &'i ForegroundEvent>,
    ) {
        let mut snapshot = self.frame_snapshot.borrow_mut();
        for event in events {
    ...
    pub fn foreground_work(&self) -> Option<ForegroundWorkSummary> {
        ...
        Some(ForegroundWorkSummary {
            ...
            max,
            p50: Duration::from_nanos(
                foreground_work
                    .histogram
                    .value_at_quantile(0.50),
            ),
            p90: Duration::from_nanos(
                foreground_work
                    .histogram
            ...
            frame_budget_overruns_total: self
                .total_budget_overruns(&foreground_work.histogram),
...
impl WindowFrameSnapshot {
    fn new() -> Self {
        Self {
            ...
            invalidations_per_frame: Histogram::new(3)
                .expect("3 significant digits is valid"),
...
impl<Output> Drop for MeasuredTaskOutput<Output> {
    fn drop(&mut self) {
        ...
        self.report
            .record_frame_timings(events.frame_events.iter());
        self.report
...
impl<'a, 'measurement> BenchAppContext<'a, 'measurement> {
    ...
    pub fn bench_task<Output>(
        &mut self,
        mut benchmark: impl FnMut(&mut Self) -> Task<Output>,
    ) where
        Output: 'static,
    {
    ...
    fn take_bencher(
        &self,
        benchmark_kind: &str,
    ) -> &'a mut criterion::Bencher<'measurement> {
...
impl VisualContext for BenchWindowContext<'_, '_> {
    ...
    fn update_window_entity<T: 'static, R>(
        ...
        self.cx
            ...
            .ok_or_else(|| {
                anyhow!(
                    "entity has no current window; use `update` instead of `update_in`"
                )
            })
    }
...
mod tests {
    ...
    fn task_completion_supports_non_send_foreground_output() {
        ...
        let task = foreground_executor.spawn(async move {
            receiver
                .await
                .expect("background task should send a value");
            task_output
        });
