...
impl EntityMap {
        ...
        T: 'static,
    {
        ...
        let handle = slot.0;
        self.entities
            .insert(handle.entity_id, Box::new(entity));
        handle
    }
...
#[cfg(any(test, feature = "leak-detection"))]
static LEAK_BACKTRACE: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
    std::env::var("LEAK_BACKTRACE").is_ok_and(|b| !b.is_empty())
});
