...
use crate::{
    Bounds, Capslock, Context, Empty, IntoElement, Keystroke, LongPressEvent, Modifiers,
    Pixels, Point, Render, TouchDragEvent, Window, point, seal::Sealed,
};
...
impl ClickEvent {
        ...
        match self {
            ...
            ClickEvent::Mouse(event) => {
                event.down.button == MouseButton::Right
                    && event.up.button == MouseButton::Right
            }
    ...
    pub fn is_middle_click(&self) -> bool {
        match self {
            ClickEvent::Keyboard(_) => false,
            ClickEvent::Mouse(event) => {
                event.down.button == MouseButton::Middle
                    && event.up.button == MouseButton::Middle
            }
            ClickEvent::Touch(_) => false,
        }
    }
    ...
    pub fn pixel_delta(&self, line_height: Pixels) -> Point<Pixels> {
        match self {
            ...
            ScrollDelta::Lines(delta) => {
                point(line_height * delta.x, line_height * delta.y)
            }
        }
    }
            ...
            div().id("testview").child(
                div()
                    ...
                    .on_action(cx.listener(
                        |this: &mut TestView, _: &TestAction, _, _| {
