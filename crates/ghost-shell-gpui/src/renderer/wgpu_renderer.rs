...
use crate::renderer::{CompositorGpuHint, WgpuAtlas, WgpuContext};
...
/// The dual-source blending directive must precede the shared shader declarations.
...
impl WgpuRenderer {
    ...
    fn new_internal(
        ...
        let gamma_offset =
            (path_globals_offset + globals_size).next_multiple_of(uniform_alignment);
    ...
    fn create_pipelines(
        ...
        let shader_source = { STORAGE_BUFFER_SHADERS };
            ...
            |name: &str,
             ...
             module: &wgpu::ShaderModule| {
                let mut bind_group_layouts =
                    vec![Some(globals_layout), Some(data_layout)];
    ...
    fn draw_sprites(
        ...
        let texture = self
            .create_texture_bind_group("atlas_texture_bind_group", &texture_info.view);
        pass.set_pipeline(pipeline);
        pass.set_bind_group(0, &self.resources().globals_bind_group, &[]);
    ...
    fn draw_paths_to_intermediate(
        ...
        let resources = self.resources();
        let Some(path_intermediate_view) = resources.path_intermediate_view.as_ref()
        else {
            return Ok(false);
        };

        let (target_view, resolve_target) =
            if let Some(ref msaa_view) = resources.path_msaa_view {
    ...
    fn write_instance_binding<T>(
        ...
        *instance_offset = offset + allocation_size;
        ...
        if !data.is_empty() {
            resources
                .queue
                .write_buffer(&resources.instance_data, offset, data);
        }
        let bind_group = resources
            ...
            .create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                        buffer: &resources.instance_data,
                        offset,
                        size: NonZeroU64::new(size),
                    }),
                }],
            });
        Ok(InstanceBinding { bind_group })
    }
    ...
    fn grow_instance_data(&mut self, required: u64) -> Result<()> {
        ...
        // Bind groups created earlier in the frame keep the previous buffer alive, so allocations written before the grow remain valid;
...
fn create_surface(
    ...
    // Safety: the caller guarantees the window handle outlives the renderer,
    // and the surface is dropped before the window is destroyed.
    unsafe {
        instance
