...
impl Keymap {
    ...
    pub fn bindings_for_input(
        ...
        let mut matched_bindings =
            SmallVec::<[(usize, BindingIndex, &KeyBinding); 1]>::new();
        ...
        for (_, ix, binding) in matched_bindings {
            ...
            if is_no_action(&*binding.action) {
                no_action_meta =
                    Some(no_action_meta.map_or(meta, |existing| existing.min(meta)));
                continue;
            }
...
mod tests {
    ...
    fn test_keymap() {
        ...
        assert_eq!(
            keymap
                .binding_enabled(&bindings[0], &[KeyContext::parse("terminal").unwrap()]),
            Some(1)
        );
        ...
        assert_eq!(
            keymap
                .binding_enabled(&bindings[1], &[KeyContext::parse("barf x=y").unwrap()]),
            None
        );
        assert_eq!(
            keymap
                .binding_enabled(&bindings[1], &[KeyContext::parse("pane x=y").unwrap()]),
            Some(1)
        );
    ...
    fn test_multiple_keystroke_binding_disabled() {
        ...
        let space_editor =
            keymap.bindings_for_input(&[space()], &editor_workspace_context());
        ...
        let space_w_editor =
            keymap.bindings_for_input(&space_w, &editor_workspace_context());
        ...
        let space_w_w_workspace =
            keymap.bindings_for_input(&space_w_w, &workspace_context());
        assert!(!space_w_w_workspace.0.is_empty());
        assert!(!space_w_w_workspace.1);

        let space_w_w_editor =
            keymap.bindings_for_input(&space_w_w, &editor_workspace_context());
    ...
    fn test_bindings_for_action_removes_binding_for_broader_targeted_unbind() {
        ...
        assert!(
            keymap
                .bindings_for_action(&ActionAlpha {})
                .next()
                .is_none()
        );
    }
