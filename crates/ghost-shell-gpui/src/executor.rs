...
impl BackgroundExecutor {
        ...
        R: Send + 'static,
    {
        if priority == Priority::RealtimeAudio {
            ...
            self.inner
                .spawn_with_priority(priority, future)
        }
    }
    ...
    pub fn allow_parking(&self) {
        ...
        if std::env::var("GPUI_RUN_UNTIL_PARKED_LOG")
            .ok()
            .as_deref()
            == Some("1")
        {
...
impl ForegroundExecutor {
        ...
        let (scheduler, session_id): (Arc<dyn Scheduler>, _) =
            if let Some(test_dispatcher) = dispatcher.as_test() {
                ...
                return Self {
                    ...
                    dispatcher,
                    #[cfg(feature = "profiler")]
                    foreground_runnables: Some(
                        platform_scheduler.foreground_runnable_counter(),
                    ),
    ...
    pub fn block_test<R>(&self, future: impl Future<Output = R>) -> R {
        ...
        output
            .take()
            .expect("block_test future did not complete")
    }
...
mod test {
    ...
    fn create_test_app() -> (TestDispatcher, BackgroundExecutor, Rc<crate::AppCell>) {
        ...
        let platform =
            TestPlatform::new(background_executor.clone(), foreground_executor);
