...
use crate::{
    ...
    Bounds, ClipboardItem, ClipboardReadError, CursorStyle, DispatchPhase, DisplayId,
    EventEmitter, ExternalDragPayload, FocusHandle, FocusMap, ForegroundExecutor, Global,
    KeyBinding, KeyContext, Keymap, Keystroke, LayoutId, Menu, MenuItem, MissingGlyph,
    OwnedMenu, PathPromptOptions, Pixels, Platform, PlatformDisplay,
    PlatformKeyboardLayout, PlatformKeyboardMapper, Point, Priority, PromptBuilder,
    PromptButton, PromptHandle, PromptLevel, Render, RenderImage, RenderablePromptHandle,
    Reservation, SubscriberSet, Subscription, SvgRenderer, SystemNotification,
    SystemNotificationResponse, Task, TextRenderingMode, TextSystem, ThermalState,
    Window, WindowAppearance, WindowButtonLayout, WindowHandle, WindowId,
    WindowInvalidator,
...
impl Application {
    ...
    pub fn new_inaccessible(platform: Rc<dyn Platform>) -> Self {
        ...
        this.0
            .borrow_mut()
            .accessibility_force_disabled = true;
        this
    }
    ...
    pub fn path_for_auxiliary_executable(&self, name: &str) -> Result<PathBuf> {
        self.0
            .borrow()
            .path_for_auxiliary_executable(name)
    }
}
...
impl MissingGlyphCallbackSlot {
    fn replace(&self, callback: MissingGlyphCallback) -> Rc<()> {
        ...
        self.entry
            .borrow_mut()
            .replace(MissingGlyphCallbackEntry {
    ...
    fn invoke(&self, missing_glyphs: &[MissingGlyph], cx: &mut App) {
        let Some((registration, mut callback)) = self
            .entry
            .borrow_mut()
            .as_mut()
            .and_then(|entry| {
...
impl App {
    ...
    pub fn refresh_windows(&mut self) {
        self.pending_effects
            .push_back(Effect::RefreshWindows);
    }
    ...
    pub(crate) fn detect_accessed_entities<R>(
        ...
        let accessed_entities_start = self
            .entities
            .accessed_entities
            .get_mut()
            .clone();
        ...
        self.update(|cx| {
            ...
            match Window::new(handle.into(), options, cx) {
                Ok(mut window) => {
                    ...
                    window.defer(cx, |window: &mut Window, cx| {
    ...
    pub fn set_window_appearance(&self, appearance: Option<WindowAppearance>) {
        self.platform
            .set_window_appearance(appearance);
    }
    ...
    pub fn prompt_for_new_path(
        ...
        self.platform
            .prompt_for_new_path(directory, suggested_name)
    }
    ...
    pub fn on_missing_glyphs(
        ...
        self.text_system
            .enable_missing_glyph_reporting();
    ...
    pub fn set_global<G: Global>(&mut self, global: G) {
        ...
        self.push_effect(Effect::NotifyGlobalObservers { global_type });
        self.globals_by_type
            .insert(global_type, Box::new(global));
    }
    ...
    pub(crate) fn lease_global<G: Global>(&mut self) -> GlobalLease<G> {
        GlobalLease::new(
            self.globals_by_type
                .remove(&TypeId::of::<G>())
                .with_context(|| {
                    format!("no global registered of type {}", type_name::<G>())
                })
                .unwrap(),
        )
    }
    ...
    pub(crate) fn end_global_lease<G: Global>(&mut self, lease: GlobalLease<G>) {
        ...
        self.push_effect(Effect::NotifyGlobalObservers { global_type });
        self.globals_by_type
            .insert(global_type, lease.global);
    }
        ...
        T: 'static,
    {
        ...
        self.observe_release(handle, move |entity, cx| {
            let _ =
                window_handle.update(cx, |_, window, cx| on_release(entity, window, cx));
        })
    }
    ...
    pub fn action_schema_by_name(
        ...
        self.actions
            .action_schema_by_name(name, generator)
    }

    /// Get a map from a deprecated action name to the canonical name.
    pub fn deprecated_actions_to_preferred_actions(
        &self,
    ...
    pub fn on_app_restart(
        &self,
        mut on_restart: impl 'static + FnMut(&mut App),
    ) -> Subscription {
    ...
    pub fn on_window_closed(
        ...
        let (subscription, activate) = self
            .window_closed_observers
            .insert((), Box::new(on_closed));
        activate();
        subscription
    }
    ...
    pub fn is_action_available(&mut self, action: &dyn Action) -> bool {
        ...
        if let Some(window) = self.active_window()
            && let Ok(window_action_available) = window
                .update(self, |_, window, cx| window.is_action_available(action, cx))
        {
    ...
    pub fn set_menus(&self, menus: impl IntoIterator<Item = Menu>) {
        ...
        self.platform
            .set_menus(menus, &self.keymap.borrow());
    }
    ...
    pub fn set_dock_menu(&self, menus: Vec<MenuItem>) {
        self.platform
            .set_dock_menu(menus, &self.keymap.borrow())
    }
    ...
    pub fn perform_dock_menu_action(&self, action: usize) {
        self.platform
            .perform_dock_menu_action(action);
    }
    ...
    pub fn update_jump_list(
        ...
        self.platform
            .update_jump_list(menus, entries)
    }
    ...
    pub fn active_drag_cursor_style(&self) -> Option<CursorStyle> {
        self.active_drag
            .as_ref()
            .and_then(|drag| drag.cursor_style)
    }
    ...
    pub(crate) fn hand_active_drag_to_platform(
        &mut self,
        source_window: WindowId,
    ) -> bool {
    ...
    pub(crate) fn hand_restored_drag_to_platform(
        &mut self,
        source_window: WindowId,
    ) -> bool {
        let Some(platform_drag) = self
    ...
    pub(crate) fn asset_entry<A: Asset>(
        &mut self,
        source: &A::Source,
    ) -> &CachedLoad<A::Output> {
        let asset_id = (TypeId::of::<A>(), hash(source));
    ...
    {
        self.inspector_element_registry
            .register(factory);
    }
                ...
                if window.removed {
                    ...
                    if let Some(tracked) = cx.tracked_entities.remove(&window_id) {
                        for entity_id in tracked {
                            ...
                            if cx.current_window_by_entity.get(&entity_id)
                                == Some(&window_id)
                            {
    ...
    fn update_entity_erased(
        ...
        self.update(|cx| {
            ...
            cx.entities
                .end_lease_erased(handle.entity_id, lease);
        });
    }
...
mod test {
    ...
    async fn test_restart_preserves_path_and_arguments(cx: &mut TestAppContext) {
        ...
        let (path, restart_arguments) = restart
            .await
            .expect("restart was not requested");
