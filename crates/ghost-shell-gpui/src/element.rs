...
use crate::{
    A11ySubtreeBuilder, App, ArenaBox, AvailableSpace, Bounds, Context, DispatchNodeId,
    ElementId, FocusHandle, InspectorElementId, LayoutId, Pixels, Point, Size, Style,
    Window, util::FluentBuilder, window::with_element_arena,
};
                    ...
                    if let Some(global_id) = global_id.as_ref() {
                        if let Some(role) = self.element.a11y_role() {
                            ...
                            node.set_bounds(accesskit::Rect {
                                ...
                                x1: ((bounds.origin.x.0 + bounds.size.width.0) * scale)
                                    as f64,
                                y1: ((bounds.origin.y.0 + bounds.size.height.0) * scale)
                                    as f64,
                            });
