...
use crate::{
    ...
    EdgesRefinement, Font, FontFallbacks, FontFeatures, FontStyle, FontWeight,
    ...
    pub fn get_bounds(
        ...
        match self {
            ...
            ObjectFit::ScaleDown => {
                ...
                if image_size.width > bounds.size.width
                    || image_size.height > bounds.size.height
                {
                    ...
                    Bounds {
                        origin: point(
                            bounds.origin.x + (bounds.size.width - new_size.width) / 2.0,
                            bounds.origin.y
                                + (bounds.size.height - new_size.height) / 2.0,
                        ),
                        size: new_size,
                    }
                } else {
                    ...
                    Bounds {
                        origin: point(
                            bounds.origin.x
                                + (bounds.size.width - original_size.width) / 2.0,
                            bounds.origin.y
                                + (bounds.size.height - original_size.height) / 2.0,
                        ),
                        size: original_size,
                    }
                }
...
#[derive(
    Default, Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize, JsonSchema,
)]
pub enum Visibility {
...
impl TextStyle {
    ...
    pub fn line_height_in_pixels(&self, rem_size: Pixels) -> Pixels {
        self.line_height
            .to_pixels(self.font_size, rem_size)
            .round()
    }
...
impl Style {
    ...
    pub fn has_opaque_background(&self) -> bool {
        self.background.as_ref().is_some_and(|fill| {
            fill.color()
...
#[derive(
    Copy, Clone, PartialEq, Eq, Debug, Default, Serialize, Deserialize, JsonSchema,
)]
// Copy of taffy::style type of the same name, to derive JsonSchema.
pub enum Display {
