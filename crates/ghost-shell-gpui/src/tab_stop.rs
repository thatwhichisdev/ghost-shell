...
impl Ord for TabStopNode {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.path.cmp(&other.path).then(
            self.node_insertion_index
                .cmp(&other.node_insertion_index),
        )
    }
}
...
impl TabStopMap {
    pub fn insert(&mut self, focus_handle: &FocusHandle) {
        ...
        self.by_id
            .insert(focus_handle.id, order.clone());
    ...
    pub fn end_group(&mut self) {
        self.insertion_history
            .push(TabStopOperation::GroupEnd);
...
mod tests {
    ...
    fn test_tab_non_stop_filtering() {
        ...
        let tab_non_stop_1 = FocusHandle::new(&focus_map)
            .tab_stop(false)
            .tab_index(1);
        let tab_stop_2 = FocusHandle::new(&focus_map)
            .tab_stop(true)
            .tab_index(2);
    ...
    impl TabStopMapTest {
        ...
        fn tab_stop(mut self, index: isize, expected: usize) -> Self {
            ...
            self.expected
                .sort_by_key(|(expected, _)| *expected);
            self
        }
        ...
        fn tab_group(
            mut self,
            tab_index: isize,
            children: impl FnOnce(Self) -> Self,
        ) -> Self {
        ...
        fn assert(self) {
            ...
            let reversed_found =
                self.traverse_tab_map(|tab_map, prev| tab_map.prev(prev));
            expected.reverse();
