...
pub struct Globals {
    ...
    pub primary_selection_manager: Option<
...
impl WaylandClientStatePtr {
    ...
    pub fn start_external_drag(
        ...
        let source = data_device_manager
            .create_data_source(&state.globals.qh, DataSourceKind::Drag);
        ...
        let changed = if let Some(keymap_state) = &state.keymap_state {
            ...
            if changed {
                state.keyboard_layout =
                    LinuxKeyboardLayout::new(layout_name.to_string().into());
            }
            changed
        } else {
            ...
            if changed {
                state.keyboard_layout =
                    LinuxKeyboardLayout::new(UNKNOWN_KEYBOARD_LAYOUT_NAME);
            }
            changed
        };

        if changed
            && let Some(mut callback) = state
                .common
                .callbacks
...
impl WaylandClient {
    pub(crate) fn new() -> Self {
        ...
        let (common, main_receiver, power_receiver) =
        ...
        let data_device =
            globals
                .data_device_manager
                .as_ref()
                .map(|data_device_manager| {
                    data_device_manager.get_data_device(&seat, &qh, ())
                });

        let primary_selection = globals
            .primary_selection_manager
            .as_ref()
            .map(|primary_selection_manager| {
        ...
        // Shell startup needs the initial outputs before constructing its windows.
        let mut state_ptr = WaylandClientStatePtr(Rc::downgrade(&state));
        if let Err(error) = event_queue.roundtrip(&mut state_ptr) {
            log::error!("Failed to discover initial Wayland outputs: {error:#}");
        }
...
impl LinuxClient for WaylandClient {
    ...
    fn open_window(
        ...
        let (session_lock, session_lock_output_id, session_lock_to_rollback) =
            match &params.kind {
                    ...
                    match state.session_lock.as_ref() {
                        ...
                        None => {
                            let manager = state
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "compositor does not support ext-session-lock-v1"
                                    )
                                })?;
        ...
        if let Some(output_id) = session_lock_output_id {
            ...
            let previous = session_lock
                .surfaces
                .insert(output_id, surface_id.clone());
        ...
        state
            .windows
            .insert(surface_id, window.0.clone());
    ...
    fn write_to_primary(&self, item: gpui::ClipboardItem) {
        ...
        if state.mouse_focused_window.is_some() || state.keyboard_focused_window.is_some()
        {
            ...
            let data_source =
                primary_selection_manager.create_source(&state.globals.qh, ());
...
impl Dispatch<zwp_linux_dmabuf_feedback_v1::ZwpLinuxDmabufFeedbackV1, ()>
    for DmabufProbeState
{
    fn event(
        state: &mut Self,
...
fn detect_compositor_gpu() -> Option<CompositorGpuHint> {
    ...
    let (globals, mut event_queue) =
        registry_queue_init::<DmabufProbeState>(&connection).ok()?;
...
impl Dispatch<wl_output::WlOutput, ()> for WaylandClientStatePtr {
    fn event(
        ...
        let Some(in_progress_output) = state
            .in_progress_outputs
            .get_mut(&output.id())
        else {
            return;
...
impl Dispatch<zwlr_layer_surface_v1::ZwlrLayerSurfaceV1, ObjectId>
    ...
    fn event(
        ...
        match event {
            ext_session_lock_surface_v1::Event::Configure {
                ...
                let Some(window) = get_window(&mut state, surface_id) else {
                    log::warn!(
                        "received configure for unknown Wayland session-lock surface"
                    );
                    return;
                };
        ...
        match event {
            ext_session_lock_v1::Event::Locked => {
                ...
                if session_lock.status == SessionLockStatus::Locked {
                    log::warn!(
                        "received duplicate locked event for Wayland session lock"
                    );
                    return;
                }
            ...
            ext_session_lock_v1::Event::Finished => {
                let Some(session_lock) = state.session_lock.take() else {
                    return;
                };
...
impl Dispatch<xdg_activation_token_v1::XdgActivationTokenV1, ()>
    for WaylandClientStatePtr
{
    fn event(
        this: &mut Self,
        ...
        if let xdg_activation_token_v1::Event::Done { token } = event {
            ...
            match state.pending_activation.take() {
                ...
                None => {
                    log::error!("activation token received with no pending activation")
                }
            }
        }
        ...
        } = event
        {
            ...
            if capabilities.contains(wl_seat::Capability::Pointer) {
                ...
                state.cursor_shape_device = state
                    ...
                    .as_ref()
                    .map(|cursor_shape_manager| {
                        cursor_shape_manager.get_pointer(&pointer, qh, ())
                    });
        ...
        match event {
            ...
            wl_keyboard::Event::Key {
                ...
                if key_state == wl_keyboard::KeyState::Pressed {
                    state
                        .serial_tracker
                        .update(SerialKind::KeyPress, serial);
                }
                            ...
                            match compose.status() {
                                ...
                                xkb::Status::Cancelled => {
                                    ...
                                    if let Some(current_key) = new_pre_edit {
                                        focused_window.handle_ime(
                                            ImeInput::SetMarkedText(current_key),
                                        );
                                    }
                                    compose.feed(keysym);
                                    state = client.borrow_mut();
                            ...
                            .insert_source(Timer::from_duration(state.repeat.delay), {
                                ...
                                move |event_timestamp, _metadata, this| {
                                    ...
                                    TimeoutAction::ToInstant(
                                        event_timestamp + repeat_interval,
                                    )
                                }
                            })
                            .unwrap();
    ...
    fn event(
        ...
        match event {
            ...
            zwp_text_input_v3::Event::Done { serial } => {
                let last_serial = state
                    .serial_tracker
                    .get(SerialKind::InputMethod);
                state
                    .serial_tracker
    ...
    fn event(
        ...
        match event {
            wl_pointer::Event::Enter {
                ...
                state
                    .serial_tracker
                    .update(SerialKind::MouseEnter, serial);
            ...
            wl_pointer::Event::Motion {
                ...
                state.mouse_location =
                    Some(point(px(surface_x as f32), px(surface_y as f32)));
                state.restore_cursor_after_hide();
            ...
            wl_pointer::Event::Button {
                ...
                if button_state == wl_pointer::ButtonState::Pressed {
                    state
                        .serial_tracker
                        .update(SerialKind::MousePress, serial);
                }
...
impl Dispatch<zwp_pointer_gestures_v1::ZwpPointerGesturesV1, ()>
    for WaylandClientStatePtr
{
    fn event(
        _this: &mut Self,
...
impl Dispatch<wp_fractional_scale_v1::WpFractionalScaleV1, ObjectId>
    for WaylandClientStatePtr
{
    fn event(
        this: &mut Self,
    ...
    fn event(
        ...
        match event {
            ...
            wl_data_device::Event::Enter {
                ...
                state
                    .serial_tracker
                    .update(SerialKind::DataDevice, serial);
                    ...
                    state
                        ...
                        .spawn(async move {
                            let file_list = match read_task.await {
                                ...
                                Err(err) => {
                                    log::error!(
                                        "error reading drag and drop pipe: {err:?}"
                                    );
                                    return;
                                }
                            };
                            ...
                            let paths: SmallVec<[_; 2]> = file_list
                                ...
                                .filter_map(|url| match url.to_file_path() {
                                    ...
                                    Err(()) => {
                                        log::error!(
                                            "Failed turn {url:?} into a file path"
                                        );
                                        None
                                    }
                                })
...
mod tests {
    ...
    fn record_entered(
        ...
        if let Some((window, PlatformInput::FileDrop(FileDropEvent::Entered { .. }))) =
            input
        {
    ...
    fn builds_empty_uri_list_without_convertible_paths() {
        ...
        assert!(
            file_uri_list(&FileDragPaths::new([(PathBuf::from("relative"), false)]))
                .is_empty()
        );
    }
