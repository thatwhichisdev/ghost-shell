...
use gpui::{
    ...
    WindowDecorations, WindowKind, WindowParams, WindowVisibility,
...
use crate::renderer::{CompositorGpuHint, WgpuRenderer, WgpuSurfaceConfig, wgpu};
...
impl WaylandSurfaceState {
    fn new(
        ...
        if let WindowKind::SessionLock = &params.kind {
            ...
            let output = target_output.as_ref().ok_or_else(|| {
                anyhow::anyhow!("session lock window requires an output")
            })?;

            let lock_surface =
                lock.get_lock_surface(surface, output, &globals.qh, surface.id());
        ...
        if let WindowKind::AnchoredPopup(options) = &params.kind {
            ...
            let xdg_popup = if let Some(parent_layer_surface) = parent.layer_surface() {
                let xdg_popup =
                    xdg_surface.get_popup(None, &positioner, &globals.qh, surface.id());
...
fn build_popup_positioner(
    ...
    let positioner = globals
        .wm_base
        .create_positioner(&globals.qh, ());
...
impl WaylandSurfaceState {
    fn ack_configure(&self, serial: u32) {
        match self {
            ...
            WaylandSurfaceState::SessionLock(state) => {
                state.lock_surface.ack_configure(serial)
            }
    ...
    fn reposition_popup(
            ...
            && xdg_popup.version() >= xdg_popup::REQ_REPOSITION_SINCE
        {
            ...
            let positioner =
                build_popup_positioner(globals, options, size, parent_geometry);
    ...
    fn destroy(&mut self) {
        match self {
            ...
            WaylandSurfaceState::SessionLock(state) => state.lock_surface.destroy(),
            WaylandSurfaceState::Popup(WaylandPopupSurfaceState {
                xdg_surface,
                xdg_popup,
                ..
...
impl WaylandWindowState {
    pub(crate) fn new(
        ...
        client: WaylandClientStatePtr,
        globals: Globals,
        gpu_context: crate::renderer::GpuContext,
...
impl PresentationState {
    ...
    fn failed(self) -> Self {
        match self {
            Self::Unpresented | Self::RetryBeforeFirstPresent => {
                Self::RetryBeforeFirstPresent
            }
...
impl WaylandWindow {
    ...
    pub fn new(
        ...
        // A lock surface's first commit must include a buffer of the configured size.
...
impl WaylandWindowStatePtr {
    ...
    fn complete_frame(&self) {
        ...
        if state.presentation.requires_presentation() {
                ...
                && state.presentation == PresentationState::RetryAfterPresent
            {
                ...
                self.frame_loop
                    .set(FrameLoop::AwaitingCallback);
                return;
            }

            self.frame_loop
                .set(FrameLoop::RetryScheduled);
            let surface_id = state.surface.id();
            let client = state.client.clone();
            drop(state);
    ...
    pub fn schedule_frame(&self) {
        match self.frame_loop.get() {
            ...
            FrameLoop::Ticking => {
                self.frame_loop
                    .set(FrameLoop::RescheduleRequested);
            }
        ...
        if let xdg_surface::Event::Configure { serial } = event {
            {
                ...
                if let Some(window_controls) = state.in_progress_window_controls.take() {
                    ...
                    if let Some(appearance_changed) =
                        callbacks.appearance_changed.as_mut()
                    {
                        appearance_changed();
                    }
                }
    ...
    pub fn handle_toplevel_decoration_event(
        &self,
    ...
    pub fn handle_toplevel_event(&self, event: xdg_toplevel::Event) -> bool {
        match event {
            ...
            xdg_toplevel::Event::WmCapabilities { capabilities } => {
                ...
                let states =
                    extract_states::<xdg_toplevel::WmCapabilities>(&capabilities);
    ...
    pub fn handle_popup_event(&self, event: xdg_popup::Event) -> bool {
        match event {
            ...
            xdg_popup::Event::Configure { width, height, .. } => {
                ...
                self.state.borrow_mut().in_progress_configure =
                    Some(InProgressConfigure {
                        size,
                        fullscreen: false,
                        maximized: false,
                        resizing: false,
    ...
    pub fn set_size_and_scale(&self, size: Option<Size<Pixels>>, scale: Option<f32>) {
        let (size, scale) = {
            ...
            state
                .renderer
                .update_drawable_size(device_bounds.size);
    ...
    pub fn set_button_layout(&self) {
        ...
        if let Some(mut fun) = callback {
            ...
            self.callbacks
                .borrow_mut()
                .button_layout_changed = Some(fun);
        }
    }
...
fn extract_states<'a, S: TryFrom<u32> + 'a>(
    states: &'a [u8],
...
impl PlatformWindow for WaylandWindow {
        ...
        if matches!(state.surface_state, WaylandSurfaceState::Popup(_)) {
            if self.0.is_configured() {
                let parent_geometry = state
                    ...
                    .unwrap_or_default();
                state.surface_state.reposition_popup(
                    &state.globals,
                    size,
                    parent_geometry,
                );
    ...
    fn activate(&self) {
        ...
        let state = self.borrow();
        if let (Some(activation), Some(app_id)) =
            (&state.globals.activation, state.app_id.clone())
        {
            state
                .client
                .set_pending_activation(state.surface.id());
            let token = activation.get_activation_token(&state.globals.qh, ());
            // The serial isn't exactly important here, since the activation is probably going to be rejected anyway.
            let serial = state
                .client
                .get_serial(SerialKind::MousePress);
    ...
    fn on_input(
        &self,
        callback: Box<dyn FnMut(PlatformInput) -> gpui::DispatchEventResult>,
    ) {
    ...
    fn on_visibility_change(&self, callback: Box<dyn FnMut(WindowVisibility)>) {
        self.0
            .callbacks
            .borrow_mut()
            .visibility_change = Some(callback);
    }
    ...
    fn on_hover_status_change(&self, callback: Box<dyn FnMut(bool)>) {
        self.0
            .callbacks
            .borrow_mut()
            .hover_status_change = Some(callback);
    }
    ...
    fn on_button_layout_changed(&self, callback: Box<dyn FnMut()>) {
        self.0
            .callbacks
            .borrow_mut()
            .button_layout_changed = Some(callback);
    }
    ...
    fn draw(&self, scene: &Scene) {
        ...
        if state.renderer.draw(scene) {
            ...
            self.0
                .frame_loop
                .set(FrameLoop::PresentationFailed);
        }
    ...
    fn start_external_drag(&self, payload: &ExternalDragPayload) -> bool {
        ...
        state
            .client
            .start_external_drag(&state.surface, payload)
    }
