...
use gpui::{
    Action, ActivityGuard, AnyWindowHandle, BackgroundExecutor, ClipboardItem,
    CursorStyle, DisplayId, ForegroundExecutor, Keymap, Menu, MenuItem, OwnedMenu,
    PathPromptOptions, Platform, PlatformDisplay, PlatformKeyboardLayout,
    PlatformKeyboardMapper, PlatformTextSystem, PlatformWindow, Result, RunnableVariant,
    ...
    pub fn new(
        ...
        let common = LinuxCommon {
            ...
            system_notifications:
                crate::linux::system_notifications::SystemNotificationState::new(),
            power_sender,
        ...
        if !self.power_listener_started {
            smol::spawn({
                ...
                async move {
                    ...
                    {
                        log::debug!(
                            "failed to listen for system sleep/wake events: {error:?}"
                        );
                    }
                }
            })
...
impl<P: LinuxClient + 'static> Platform for LinuxPlatform<P> {
    ...
    fn text_system(&self) -> Arc<dyn PlatformTextSystem> {
        self.inner
            .with_common(|common| common.text_system.clone())
    }
    ...
    fn quit(&self) {
        self.inner
            .with_common(|common| common.signal.stop());
    }
    ...
    fn get_menus(&self) -> Option<Vec<OwnedMenu>> {
        self.inner
            .with_common(|common| Some(common.menus.clone()))
    }
    ...
    fn should_auto_hide_scrollbars(&self) -> bool {
        self.inner
            .with_common(|common| common.auto_hide_scrollbars)
    }

    fn window_appearance(&self) -> WindowAppearance {
        self.inner
            .with_common(|common| common.appearance)
    }

    fn button_layout(&self) -> Option<WindowButtonLayout> {
        Some(
            self.inner
                .with_common(|common| common.button_layout),
        )
    }
            ...
            .spawn(async move {
                ...
                if xdg_open_failed {
                    match ashpd::desktop::open_uri::OpenFileRequest::default()
                        .activation_token(
                            activation_token.map(ashpd::ActivationToken::from),
                        )
                        .send_uri(&uri)
                        .await
...
pub(super) fn cursor_style_to_icon_names(style: CursorStyle) -> &'static [&'static str] {
    ...
    match style {
        ...
        CursorStyle::ResizeUpLeftDownRight => {
            &["size_fdiag", "bd_double_arrow", "nwse-resize"]
        }
...
pub(super) fn capslock_from_xkb(keymap_state: &State) -> gpui::Capslock {
    let on =
        keymap_state.mod_name_is_active(xkb::MOD_NAME_CAPS, xkb::STATE_MODS_EFFECTIVE);
    gpui::Capslock { on }
}
...
mod tests {
    ...
    mod idle_sleep_prevention {
        ...
        async fn guard_passes_through_and_releases_on_drop(cx: &mut TestAppContext) {
            ...
            assert!(
                guard_tx
                    .send(Ok(release_guard(released.clone())))
                    .is_ok()
            );
        ...
        async fn acquisition_error_passes_through(cx: &mut TestAppContext) {
            ...
            assert!(
                guard_tx
                    .send(Err(anyhow!("inhibition rejected")))
                    .is_ok()
            );
            assert_eq!(
        ...
        async fn late_guard_after_timeout_is_released(cx: &mut TestAppContext) {
            ...
            cx.run_until_parked();
            cx.executor()
                .advance_clock(Duration::from_secs(9));
            cx.run_until_parked();
    ...
    mod read_fd_with_timeout {
        ...
        fn times_out_when_writer_stalls_after_partial_write() {
            ...
            let err =
                read_fd_with_timeout(pipe.read, Duration::from_millis(50)).unwrap_err();
            assert!(
                err.to_string().contains("timed out"),
                "unexpected error: {err}"
            );
