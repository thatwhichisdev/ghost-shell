...
impl SpringConfig {
    ...
    pub fn propagator(&self, delta_time: f32) -> [[f32; 2]; 2] {
        ...
        if damping_ratio < 1.0 - CRITICAL_DAMPING_TOLERANCE {
            ...
            [
                [
                    (-fast_root * slow_exponential + slow_root * fast_exponential)
                        / denominator,
                    (slow_exponential - fast_exponential) / denominator,
                ],
                [
                    slow_root * fast_root * (fast_exponential - slow_exponential)
                        / denominator,
                    (slow_root * slow_exponential - fast_root * fast_exponential)
                        / denominator,
                ],
            ]
        } else {
    ...
    pub fn settle_time(&self, state: SpringState, target: f32, epsilon: f32) -> Duration {
        ...
        if damping_ratio < 1.0 - CRITICAL_DAMPING_TOLERANCE {
            ...
            let damped_frequency =
                natural_frequency * (1.0 - damping_ratio * damping_ratio).sqrt();
            let sine_coefficient =
                (state.velocity + decay * displacement) / damped_frequency;
            let position_envelope = displacement.hypot(sine_coefficient);
            let velocity_cosine =
                damped_frequency * sine_coefficient - decay * displacement;
            let velocity_sine =
                -damped_frequency * displacement - decay * sine_coefficient;
            ...
            let fast_root = -natural_frequency * root_sum;
            let denominator = slow_root - fast_root;
            let slow_coefficient =
                (state.velocity - fast_root * displacement) / denominator;
            let fast_coefficient =
                (slow_root * displacement - state.velocity) / denominator;
            ...
            let velocity_constant =
                (linear_coefficient - natural_frequency * displacement).abs();
            let velocity_linear = natural_frequency * linear_coefficient.abs();
            let position_decay_start = envelope_decay_start(
                position_constant,
                position_linear,
                natural_frequency,
            );
            let velocity_decay_start = envelope_decay_start(
                velocity_constant,
                velocity_linear,
                natural_frequency,
            );
...
impl AnimationPhase {
    ...
    pub fn clamp(self, range: RangeInclusive<f32>) -> Self {
        ...
        Self(
            self.0
                .clamp(first.min(second), first.max(second)),
        )
    }
...
mod tests {
        ...
        for damping in [4.0, 20.0, 40.0] {
            ...
            for additional_time in [0.0, 0.1, 1.0] {
                let state = config.step(
                    initial_state,
                    3.0,
                    duration.as_secs_f32() + additional_time,
                );
                assert!(
    ...
    fn settle_time_accounts_for_motion_outside_an_instantaneous_tolerance() {
        ...
        assert!(config.is_settled(
            config.step(state, 1.0, duration.as_secs_f32()),
            1.0,
            0.125,
        ));
    }
