...
use crate::{
    ...
    ExternalDragPayloadSource, FileDropEvent, FocusHandle, Global, GlobalElementId,
    Hitbox, HitboxBehavior, HitboxId, InspectorElementId, IntoElement, IsZero,
    KeyContext, KeyDownEvent, KeyUpEvent, KeyboardButton, KeyboardClickEvent, LayoutId,
    LongPressEvent, ModifiersChangedEvent, MouseButton, MouseClickEvent, MouseDownEvent,
    MouseExitEvent, MouseMoveEvent, MousePressureEvent, MouseUpEvent, OngoingScroll,
    Overflow, ParentElement, PinchEvent, Pixels, Point, Render, ScrollWheelEvent,
    SharedString, Size, Style, StyleRefinement, Styled, Task, TooltipId, TouchPhase,
...
impl Interactivity {
    ...
    pub fn on_mouse_down(
        ...
        self.mouse_down_listeners.push(Box::new(
    ...
    pub fn on_mouse_pressure(
        ...
        self.mouse_pressure_listeners.push(Box::new(
            move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Bubble && hitbox.is_hovered(window) {
                    (listener)(event, window, cx)
                }
    ...
    pub fn on_mouse_up(
        ...
        self.mouse_up_listeners.push(Box::new(
            move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Bubble
                    && event.button == button
                    && hitbox.is_hovered(window)
    ...
    pub fn on_mouse_exit(
        ...
        self.mouse_exit_listeners.push(Box::new(
            move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Bubble && hitbox.is_hovered(window) {
                    (listener)(event, window, cx);
                }
    ...
    pub fn on_file_drop_exit(
        ...
        self.file_drop_exit_listeners.push(Box::new(
            move |event, phase, hitbox, window, cx| {
                if phase == DispatchPhase::Bubble
...
pub trait InteractiveElement: Sized {
    ...
    fn capture_any_mouse_down(
        ...
        self.interactivity()
            .capture_any_mouse_down(listener);
        self
    }
    ...
    fn capture_mouse_pressure(
        ...
        self.interactivity()
            .capture_mouse_pressure(listener);
        self
    }
    ...
    fn on_mouse_up_out(
        ...
        self.interactivity()
            .on_mouse_up_out(button, listener);
        self
    }
    ...
    fn on_boxed_action(
        ...
        self.interactivity()
            .on_boxed_action(action, listener);
        self
    }
    ...
    where
        Self: Sized,
    {
        self.interactivity().in_focus_style =
    ...
    where
        Self: Sized,
    {
        self.interactivity().focus_visible_style =
            Some(Box::new(f(StyleRefinement::default())));
        self
    }
}
...
pub trait StatefulInteractiveElement: InteractiveElement {
    ...
    {
        self.interactivity()
            .external_drag_payload(resolver);
        self
    }
...
pub(crate) type MousePressureListener = Box<
    dyn Fn(&MousePressureEvent, DispatchPhase, &Hitbox, &mut Window, &mut App) + 'static,
>;
...
pub(crate) type ScrollWheelListener = Box<
    dyn Fn(&ScrollWheelEvent, DispatchPhase, &Hitbox, &mut Window, &mut App) + 'static,
>;
...
impl HoverListenerMode {
    fn is_hovered(self, hitbox: &Hitbox, window: &Window) -> bool {
        match self {
            ...
            Self::InputModalityIndependent => hitbox
                .id
                .is_hovered_ignoring_last_input(window),
        }
    }
...
impl Element for Div {
    ...
    fn prepaint(
        ...
        let content_size = if request_layout.child_layout_ids.is_empty() {
            bounds.size
        } else if let Some(scroll_handle) = self
            .interactivity
            ...
            .as_ref()
        {
            ...
            state.child_bounds =
                Vec::with_capacity(request_layout.child_layout_ids.len());
        ...
        if let Some(scroll_handle) = self
            .interactivity
            .tracked_scroll_handle
            .as_ref()
        {
...
pub struct Interactivity {
    ...
    pub(crate) a11y_synthetic_children:
        Option<Box<dyn FnOnce(&mut crate::A11ySubtreeBuilder)>>,
...
impl Interactivity {
        ...
        window.with_optional_element_state::<InteractiveElementState, _>(
            ...
            |element_state, window| {
                ...
                if let Some(scroll_handle) = self.tracked_scroll_handle.as_ref() {
                    ...
                    self.ongoing_scroll =
                        Some(scroll_handle_state.ongoing_scroll.clone());
                    ...
                    && let Some(element_state) = element_state.as_mut()
                {
                    ...
                    self.ongoing_scroll = Some(
                        element_state
                            .ongoing_scroll
                            .get_or_insert_with(|| {
                                Rc::new(RefCell::new(OngoingScroll::default()))
                            })
                            .clone(),
                    );
                }
        ...
        if let Some(focus_handle) = self.tracked_focus_handle.as_ref() {
            ...
            if window.a11y.is_active() {
                if let Some(global_id) = global_id {
                    ...
                    window
                        .a11y
                        .set_focusable(node_id, focus_handle.id);
                ...
                if let Some(element_state) = element_state.as_mut() {
                    ...
                    if let Some(active_tooltip) = element_state.active_tooltip.as_ref() {
                        if self.tooltip_builder.is_some() {
                            self.tooltip_id =
                                set_tooltip_on_window(active_tooltip, window);
                        } else {
                            // If there is no longer a tooltip builder, remove the active tooltip.
                            if let Some(long_press_tooltip_active) = element_state
                                            ...
                                            window.with_tab_group(tab_group, |window| {
                                                ...
                                                if let Some(hitbox) = hitbox {
                                                    ...
                                                    self.paint_debug_info(
                                                        global_id, hitbox, &style,
                                                        window, cx,
                                                    );

                                                    if let Some(drag) =
                                                        cx.active_drag.as_ref()
                                                    {
            ...
            let render_debug_text = |window: &mut Window| {
                    ...
                    .and_then(|mut text| text.pop())
                {
                        ...
                        && window.modifiers().secondary()
                    {
                        ...
                        window.on_key_event({
                            move |e: &crate::ModifiersChangedEvent,
                                  _phase,
                                  window,
                                  _cx| {
    ...
    fn paint_mouse_listeners(
        ...
        if let Some(element_state) = element_state {
            ...
            if let Some(hover_listener) = self.hover_listener.take() {
                ...
                let update_hover =
                    move |is_hovered: bool, window: &mut Window, cx: &mut App| {
...

            {
                ...
                window.on_mouse_event(move |_: &MouseDownEvent, phase, window, _cx| {
                    if phase == DispatchPhase::Bubble && !window.default_prevented() {
                        let group_hovered =
                            active_group_hitbox.is_some_and(|group_hitbox_id| {
    ...
    fn compute_style_internal(
        ...
        if !cx.has_active_drag() {
            if let Some(group_hover) = self.group_hover_style.as_ref() {
                let is_group_hovered = if let Some(group_hitbox_id) =
                    GroupHitboxes::get(&group_hover.group, cx)
                {
    ...
    let action = match active_tooltip.borrow().as_ref() {
        ...
        Some(ActiveTooltip::Visible { is_hoverable, .. }) => {
            if phase.capture()
                ...
                && (!*is_hoverable
                    || !tooltip_id
                        .is_some_and(|tooltip_id| tooltip_id.is_hovered(window)))
            {
                Action::CheckVisible
            } else {
                Action::None
    ...
    E: Element,
{
    ...
    fn a11y_synthetic_children(
        ...
        self.element
            .a11y_synthetic_children(prepaint, builder);
    }
...
impl ScrollAnchor {
    ...
    pub fn scroll_to(&self, window: &mut Window, _cx: &mut App) {
        ...
        window.on_next_frame(move |_, _| {
            ...
            this.handle
                .set_offset(viewport_bounds.origin - self_bounds);
        });
    }
}
...
mod tests {
    ...
    use crate::{
        ...
        MouseMoveEvent, TestAppContext, TouchEvent, TouchId, canvas,
                            ...
                            .child(canvas(
                                ...
                                move |_, _, _, _| {
                                    anonymous_paint_count
                                        .set(anonymous_paint_count.get() + 1)
                                },
                            )),
                    )
                    .child(
    ...
    #[gpui::test]
    fn input_modality_independent_hover_listener_updates_after_key_press(
        cx: &mut TestAppContext,
    ) {
        let hover_transitions = Rc::new(RefCell::new(Vec::new()));
        let window = cx.add_window({
    ...
    type CapturedActiveTooltip =
        Rc<RefCell<Option<Weak<RefCell<Option<ActiveTooltip>>>>>>;
            ...
            window.with_global_id("target".into(), |global_id, window| {
                window.with_element_state::<InteractiveElementState, _>(
                    ...
                    |state, _window| {
                        ...
                        *self.captured_active_tooltip.borrow_mut() = state
                            .active_tooltip
                            .as_ref()
                            .map(Rc::downgrade);
                        ((), state)
    ...
    fn scroll_handle_aligns_tall_children_to_top_edge() {
        let handle = ScrollHandle::new();
        {
            ...
            state.child_bounds =
                vec![Bounds::new(point(px(0.), px(25.)), size(px(20.), px(200.)))];
    ...
    fn long_press_shows_tooltip_until_touch_ends() {
        let (mut test_app, any_window, captured_active_tooltip) =
            create_tooltip_owner_test(None);
        ...
        test_app
            ...
            .unwrap();
        test_app.dispatcher.advance_clock(
            GestureTuning::default().long_press_duration + Duration::from_millis(1),
        );
    ...
    fn keyboard_activation_fires_click_on_same_element() {
        let (mut cx, window, clicks, focus_a, _focus_b) =
            setup_keyboard_activation_test();
