...
use crate::{
    ActiveTooltip, AnyView, App, Bounds, DispatchPhase, Element, ElementId,
    ...
    SharedString, Size, TextOverflow, TextRun, TextStyle, TooltipId, TruncateFrom,
...
impl Element for Text {
    ...
    fn request_layout(
        ...
        <SharedString as Element>::request_layout(
            &mut self.text,
            id,
            inspector_id,
            window,
...
impl StyledText {
    ...
    fn apply_font_family_overrides(
        ...
        for run in runs.iter_mut() {
            ...
            while override_idx < overrides.len()
                && overrides[override_idx].0.end <= byte_offset
            {
                override_idx += 1;
            }
...
impl Element for StyledText {
    ...
    fn request_layout(
        ...
        if let Some(ref overrides) = font_family_overrides {
            let runs = runs
                .get_or_insert_with(|| vec![window.text_style().to_run(self.text.len())]);
            Self::apply_font_family_overrides(runs, overrides);
        }

        let layout_id = self
            .layout
            .layout(self.text.clone(), runs, window, cx);
        (layout_id, ())
    }
                    ...
                    if let Some(text_overflow) = text_style.text_overflow.clone() {
                        let width =
                            known_dimensions
                                .width
                                .or(match available_space.width {
                                    crate::AvailableSpace::Definite(x) => {
                        ...
                        match text_overflow {
                            ...
                            TextOverflow::TruncateStart(s) => {
                                (width, s, TruncateFrom::Start)
                            }
                            TextOverflow::TruncateMiddle(s) => {
                                (width, s, TruncateFrom::Middle)
                            }
                        }
                    } else {
        ...
        for line in &element_state.lines {
            ...
            if position.y > line_bottom {
                ...
                match line.index_for_position(position_within_line, line_height) {
                    ...
                    Err(index_within_line) => {
                        return Err(line_start_ix + index_within_line);
                    }
                }
            }
        }
    ...
    pub fn on_click(
        ...
        self.click_listener = Some(Box::new(move |ranges, event, window, cx| {
            for (range_ix, range) in ranges.iter().enumerate() {
                if range.contains(&event.mouse_down_index)
                    && range.contains(&event.mouse_up_index)
                {
            ...
            |interactive_state, window| {
                ...
                if let Some(interactive_state) = interactive_state.as_mut() {
                    if self.tooltip_builder.is_some() {
                        self.tooltip_id = set_tooltip_on_window(
                            &interactive_state.active_tooltip,
                            window,
                        );
                    } else {
                    ...
                    let build_tooltip = Rc::new({
                        ...
                        move |window: &mut Window, cx: &mut App| {
                            text_layout
                                ...
                                .ok()
                                .and_then(|position| {
                                    tooltip_builder(position, window, cx)
                                })
