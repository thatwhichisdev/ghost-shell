...
use crate::{
    ...
    SpringState, SpringTarget, Window,
};
        ...
        window.with_element_state(global_id.unwrap(), |state, window| {
            ...
            match state.playback {
                SpringPlayback::Running => {
                    state.spring = state
                        .config
                        .step(state.spring, state.target, elapsed);
                }
    ...
    fn request_layout(
        ...
        window.with_element_state(global_id.unwrap(), |state, window| {
            ...
            let (animation_ix, delta, done) = if cx.reduce_motion() {
                ...
                let elapsed =
                    if self.animations[animation_ix].synced && !duration.is_zero() {
                        let elapsed =
                            cx.background_executor().now() - cx.synced_animation_epoch;
                ...
                match self.animations[animation_ix].max_fps {
                    Some(max_fps) if max_fps.is_finite() && max_fps > 0.0 => {
                        if !state.delayed_frame_pending.get() {
                            ...
                            let delayed_frame_pending =
                                state.delayed_frame_pending.clone();
...
mod tests {
    ...
    use super::*;
    use crate::{
        Animation, Context, InteractiveElement, Pixels, Render, SpringAnimation,
        SpringConfig, TestAppContext, WindowHandle, div, prelude::*, px, size,
    };
    ...
    #[gpui::test]
    fn test_cancelled_and_completed_springs_resolve_their_endpoints(
        cx: &mut TestAppContext,
    ) {
        let rendered_values = Rc::new(RefCell::new(Vec::new()));
        let window = cx.open_window(size(px(100.0), px(100.0)), {
    ...
    fn test_max_fps_schedules_timer_driven_frames(cx: &mut TestAppContext) {
        ...
        cx.executor()
            .advance_clock(Duration::from_millis(105));
        cx.run_until_parked();
    ...
    fn test_synced_animations_share_phase_across_elements(cx: &mut TestAppContext) {
        ...
        cx.executor()
            .advance_clock(Duration::from_millis(2250));
        simulate_next_frame(&window, cx);
        ...
        cx.executor().advance_clock(
            Duration::from_secs(300 * 24 * 60 * 60) + Duration::from_millis(500),
        );
        simulate_next_frame(&window, cx);
