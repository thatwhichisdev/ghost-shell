...
use crate::{
    App, Asset, Bounds, Element, GlobalElementId, Hitbox, InspectorElementId,
    InteractiveElement, Interactivity, IntoElement, LayoutId, Pixels, Point, Radians,
    SharedString, Size, StyleRefinement, Styled, TransformationMatrix, Window, point, px,
    radians, size,
};
        ...
        Self: Sized,
    {
        self.interactivity.paint(
            ...
            |style, window, cx| {
                ...
                if let Some((data, path)) = self
                    .data
                    .as_ref()
                    .zip(self.data_path.as_ref())
                {
                ...
                } else if let Some((path, color)) = self
                    .external_path
                    .as_ref()
                    .zip(style.text.color)
                {
                    ...
                    self.path.as_ref().zip(style.text.color)
                {
                    window
