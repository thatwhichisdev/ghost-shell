...
use crate::{
    ...
    RenderImage, Resource, SharedString, SharedUri, StyleRefinement, Styled, Task,
                    ...
                    match self.source.use_data(
                        ...
                        Some(Ok(data)) => {
                            ...
                            if let Some(state) = &mut state {
                                state.frame_index =
                                    state.frame_index.min(max_frame_index);
                                if frame_count > 1 && !cx.reduce_motion() {
                                    if window.is_window_active() {
                                        let current_time = Instant::now();
                                        if let Some(last_frame_time) =
                                            state.last_frame_time
                                        {
                                            let elapsed = current_time - last_frame_time;
                                            let frame_duration = Duration::from(
                                                data.delay(state.frame_index),
                                            );
                            ...
                            if let Length::Auto = style.size.width {
                                style.size.width = match style.size.height {
                                    Length::Definite(DefiniteLength::Absolute(
                                        ...
                                        let height_px =
                                            abs_length.to_pixels(window.rem_size());
                                        Length::Definite(
...
impl ImageSource {
    ...
    pub fn is_asset_cached(&self, cx: &App) -> bool {
        match self {
            ImageSource::Resource(resource) => {
                cx.has_asset::<ImgResourceLoader>(resource)
            }
    ...
    fn load(
        ...
        async move {
            let bytes = match source.clone() {
                ...
                Resource::Uri(uri) => return Err(ImageCacheError::UnsupportedUri(uri)),
            ...
            if let Ok(format) = image::guess_format(&bytes) {
                let data = match format {
                    ImageFormat::Gif => {
                        ...
                        if frames.is_empty() {
                            return Err(ImageCacheError::Other(Arc::new(
                                anyhow::anyhow!(
                            ...
                            for frame in decoder.into_frames() {
                                match frame {
                                    Ok(mut frame) => {
                                        // Convert from RGBA to BGRA.
                                        for pixel in
                                            frame.buffer_mut().chunks_exact_mut(4)
                                        {
                                            pixel.swap(0, 2);
                                        }
                                        frames.push(frame);
...
pub enum ImageCacheError {
    ...
    #[error("IO error: {0}")]
    Io(Arc<std::io::Error>),
    /// Remote resources are deliberately unsupported by the shell framework.
    #[error("remote image loading is unsupported: {0}")]
    UnsupportedUri(SharedUri),
...
mod tests {
    ...
    #[gpui::test]
    fn remote_images_are_rejected(cx: &mut TestAppContext) {
        let result = cx.update(|cx| {
            crate::block_on(ImageAssetLoader::load(
                Resource::Uri("https://example.invalid/icon.png".into()),
                cx,
            ))
        });
        assert!(matches!(result, Err(ImageCacheError::UnsupportedUri(_))));
    }
    ...
    fn test_image_with_size(width: u32, height: u32) -> Arc<RenderImage> {
        let frame =
            Frame::new(ImageBuffer::from_pixel(width, height, Rgba([0, 0, 0, 0])));
    ...
    fn zero_frame_image_does_not_panic_on_paint(cx: &mut TestAppContext) {
        cx.add_empty_window().draw(
            point(px(0.), px(0.)),
            size(px(100.), px(100.)),
            |_, _| img(ImageSource::Render(test_image(0))).into_any_element(),
        );
    }
    ...
    #[gpui::test]
    fn explicit_aspect_ratio_is_not_overridden_by_intrinsic_ratio(
    ...
    #[gpui::test]
    fn image_object_fit_cover_clamps_corner_radii_to_visible_bounds(
        cx: &mut TestAppContext,
    ) {
        let window = cx.add_empty_window();
        window.draw(point(px(0.), px(0.)), size(px(100.), px(100.)), |_, _| {
