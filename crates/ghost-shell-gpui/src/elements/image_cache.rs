...
use crate::{
    ...
    InspectorElementId, IntoElement, LayoutId, ParentElement, Pixels, RenderImage,
    Resource, Style, StyleRefinement, Styled, Window, hash,
};
        ...
        window
            .with_global_id(self.id.clone(), |global_id, window| {
                window.with_element_state::<Entity<RetainAllImageCache>, _>(
                    ...
                    |cache, _window| {
                        let mut cache =
                            cache.unwrap_or_else(|| RetainAllImageCache::new(cx));
                        (cache.clone(), cache)
                    },
                )
            })
