...
impl ListItem {
    ...
    fn focus_handle(&self) -> Option<FocusHandle> {
        match self {
            ...
            | ListItem::Measured { focus_handle, .. } => focus_handle.clone(),
        }
    }
    ...
    fn contains_focused(&self, window: &Window, cx: &App) -> bool {
        match self {
            ListItem::Unmeasured { focus_handle, .. }
            | ListItem::Measured { focus_handle, .. } => focus_handle
                .as_ref()
...
impl ListState {
    ...
    fn remeasure_items_with_scroll_anchor(
        &self,
        range: Range<usize>,
    ...
    pub fn bounds_for_item(&self, ix: usize) -> Option<Bounds<Pixels>> {
        ...
        let mut cursor = state
            .items
            .cursor::<Dimensions<Count, Height>>(());
        cursor.seek(&Count(scroll_top.item_ix), Bias::Right);
    ...
    pub fn set_offset_from_scrollbar(&self, point: Point<Pixels>) {
        self.0
            .borrow_mut()
            .set_offset_from_scrollbar(point);
    }
...
impl StateInner {
            ...
            PendingScroll::Proportional(_) => {
                ...
                cursor
                    ...
                    .map(|size| {
                        PendingScroll::Proportional(PendingScrollFraction {
                            item_ix: scroll_top.item_ix,
                            fraction: (scroll_top.offset_in_item.0 / size.height.0)
                                .clamp(0.0, 1.0),
                        })
                    })
            }
    ...
    fn scroll(
        ...
        let scroll_max = (self.items.summary().height + padding.top + padding.bottom
            - height)
            .max(px(0.));
                        ...
                        match pending_scroll {
                                ...
                                if item_ix == scroll_top.item_ix =>
                            {
                                scroll_top.offset_in_item =
                                    offset.min(element_size.height);
                                self.logical_scroll_top = Some(scroll_top);
                            }
                                ...
                                if pending_scroll.item_ix == scroll_top.item_ix =>
                            {
                                ...
                                scroll_top.offset_in_item = Pixels(
                                    pending_scroll.fraction * element_size.height.0,
                                );
                                self.logical_scroll_top = Some(scroll_top);
                            }
                            _ => {}
            ...
            if bounds.size.height > padding.top + padding.bottom {
                ...
                for item in &mut layout_response.item_layouts {
                    window.with_content_mask(Some(ContentMask { bounds }), |window| {
                        item.element
                            .prepaint_at(item_origin, window, cx);
                    });
                        ...
                        && autoscroll
                    {
                        if autoscroll_bounds.top() < bounds.top() {
                            ...
                            let mut offset_in_item =
                                autoscroll_bounds.top() - item_origin.y;
    ...
    fn set_offset_from_scrollbar(&mut self, point: Point<Pixels>) {
        ...
        if self.alignment == ListAlignment::Bottom && new_scroll_top == scroll_max {
            ...
            let (start, _, _) = self.items.find::<ListItemSummary, _>(
                (),
                &Height(new_scroll_top),
                Bias::Right,
            );
                        ...
                        move |known_dimensions, available_space, _window, _cx| {
                            let width = known_dimensions.width.unwrap_or(
                                match available_space.width {
                                    ...
                                    AvailableSpace::MinContent
                                    | AvailableSpace::MaxContent => max_element_width,
                                },
                            );
                            let height = match available_space.height {
                                AvailableSpace::Definite(height) => {
                                    total_height.min(height)
                                }
                                AvailableSpace::MinContent
                                | AvailableSpace::MaxContent => total_height,
                            };
                            size(width, height)
                        },
                    )
...
mod test {
    ...
    use crate::{
        self as gpui, AppContext, Bounds, Context, Element, FollowMode,
        InteractiveElement, IntoElement, ListState, Render, Styled, TestAppContext,
        Window, canvas, div, list, point, px, size,
    };
    ...
    fn test_item_viewport_queries_before_logical_scroll_top(cx: &mut TestAppContext) {
        ...
        cx.draw(point(px(0.), px(0.)), size(px(100.), px(20.)), |_, cx| {
            cx.new(|_| TestListView(state.clone()))
                .into_any_element()
        });
    ...
    #[gpui::test]
    fn test_item_viewport_queries_remain_stable_with_zero_height_viewport(
    ...
    #[gpui::test]
    fn test_item_viewport_queries_after_scroll_to_end_before_layout(
        cx: &mut TestAppContext,
    ) {
        let cx = cx.add_empty_window();
    ...
    #[gpui::test]
    fn test_remeasure_then_scroll_does_not_revert_scroll_position(
        cx: &mut TestAppContext,
    ) {
        let cx = cx.add_empty_window();
    ...
    #[gpui::test]
    fn test_follow_tail_reengagement_not_fooled_by_unmeasured_items(
        cx: &mut TestAppContext,
    ) {
        let cx = cx.add_empty_window();
