...
impl ProfilingCollector {
    ...
    pub fn collect_unseen(
        ...
        for thread in all_timings {
            ...
            let buffer_start = thread
                .total_pushed
                .saturating_sub(buffer_len);
            ...
            self.cursors
                .insert(thread.thread_id, cursor_advance);
        ...
        ThreadTaskTimings {
            ...
            timings: self
                ...
                .chain(
                    self.running
                        .filter(|_| {
                            matches!(includes, TasksIncluded::CompletedAndRunning)
                        })
...
pub fn update_running_task(
    ...
    THREAD_TIMINGS.with(|timings| {
        timings
            .lock()
            .update_running_task(spawned, location);
    });
}
...
pub fn save_task_timing() {
    ...
    {
        let timing =
            THREAD_TIMINGS.with(|timings| timings.lock().save_task_timing(yielded_at));
...
#[doc(hidden)]
pub fn get_current_thread_task_timings(
    include_running: TasksIncluded,
) -> ThreadTaskTimings {
    THREAD_TIMINGS.with(|timings| {
        timings
            .lock()
            .get_thread_task_timings(include_running)
    })
}
...
impl PresentTiming {
    ...
    pub fn present_duration(&self) -> Duration {
        self.present_end
            .duration_since(self.present_start)
    }
}
...
impl WindowProfiler {
    ...
    pub fn end_input(&mut self, caused_invalidation: bool) {
        let Some(WindowActivity::Input { started_at, kind }) =
        ...
        if !caused_invalidation
            || !journal::frame_sample_is_valid(self.window_id, started_at)
        {
            return;
        }
        if self
    ...
    fn record_present_at(
        ...
        {
            let latency_nanos = present_end
                .duration_since(first_input_at)
                .as_nanos() as u64;
        ...
        let frame = self
            ...
            .take()
            .filter(|frame| {
                journal::frame_sample_is_valid(self.window_id, frame.draw_start)
            })
            .map(|mut frame| {
        ...
        if let Some(dirty_at) = frame.dirty_at
            && let Err(error) = self.dirty_to_present_histogram.record(
                present_end
...
mod tests {
    ...
    fn records_intervals_only_between_animation_frames() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(1))
    ...
    fn missed_frames_stretch_the_recorded_interval() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(2))
            .expect("window profiler should initialize");
        let start = Instant::now();
    ...
    fn ignores_re_presents_of_unchanged_frames() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(3))
            .expect("window profiler should initialize");
        let start = Instant::now();
    ...
    fn skips_intervals_for_inactive_windows() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(4))
            .expect("window profiler should initialize");
        let start = Instant::now();
    ...
    fn records_dirty_to_present_durations() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(8))
            .expect("window profiler should initialize");
        let draw_end = Instant::now();
    ...
    fn records_every_draw_duration() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(5))
            .expect("window profiler should initialize");
    ...
    fn records_input_latency_at_the_frame_presentation_timestamp() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(6))
            .expect("window profiler should initialize");
        ...
        assert!(
            snapshot.latency_histogram.max()
    ...
    fn excludes_input_that_arrives_during_a_draw() {
        let mut window_profiler = WindowProfiler::new(WindowId::from(7))
            .expect("window profiler should initialize");
