...
impl KeyContext {
    /// Initialize a new [`KeyContext`] with the Linux platform context.
    ...
    pub fn add<I: Into<SharedString>>(&mut self, identifier: I) {
        ...
        if !self.contains(&key) {
            self.0
                .push(ContextEntry { key, value: None })
        }
    }

    /// Set a key value pair in this context, if it's not already set.
    pub fn set<S1: Into<SharedString>, S2: Into<SharedString>>(
        &mut self,
        key: S1,
        value: S2,
    ) {
    ...
    pub fn contains(&self, key: &str) -> bool {
        self.0
            .iter()
            .any(|entry| entry.key.as_ref() == key)
    }
...
impl KeyBindingContextPredicate {
    ...
    pub fn eval_inner(
        ...
        match self {
            ...
            Self::And(left, right) => {
                left.eval_inner(contexts, all_contexts)
                    && right.eval_inner(contexts, all_contexts)
            }
            Self::Or(left, right) => {
                left.eval_inner(contexts, all_contexts)
                    || right.eval_inner(contexts, all_contexts)
...
mod tests {
    ...
    fn test_not_operator() {
        ...
        assert!(
            !not_predicate.eval(&[editor_context.clone(), workspace_context.clone()])
        );
        assert!(
            !not_predicate.eval(&[workspace_context.clone(), editor_context.clone()])
        );

        let complex_not =
            KeyBindingContextPredicate::parse("!editor && workspace").unwrap();
        ...
        let not_mode_predicate =
            KeyBindingContextPredicate::parse("!(mode == full)").unwrap();
        ...
        let not_descendant =
            KeyBindingContextPredicate::parse("!(parent > child)").unwrap();
        ...
        // Pane > !Workspace - should match
        let pane_not_workspace =
            KeyBindingContextPredicate::parse("Pane > !Workspace").unwrap();
        assert!(pane_not_workspace.eval(&[pane_context.clone(), editor_context.clone()]));
        assert!(
            !pane_not_workspace.eval(&[pane_context.clone(), workspace_context.clone()])
        );
