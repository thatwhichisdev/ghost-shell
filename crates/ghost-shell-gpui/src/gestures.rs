...
use crate::{
    ...
    ScrollWheelEvent, TouchEvent, TouchId, TouchPhase, point, px, seal::Sealed,
};
    ...
    fn fling_duration(self, speed: f32) -> Duration {
        match self {
            ...
            Self::FrictionSpline {
                ...
                let seconds =
                    (deceleration / (friction_spline::deceleration_rate() - 1.)).exp();
    ...
    fn fling_distance(self, speed: f32, elapsed: Duration) -> f32 {
        ...
        match self {
            Self::Exponential { decay_per_ms } => {
                ...
                (speed / 1000.) * (decay_per_ms.powf(milliseconds) - 1.)
                    / decay_per_ms.ln()
            }
            Self::FrictionSpline {
                friction,
...
mod friction_spline {
    ...
    static SPLINE_POSITION: LazyLock<[f32; NB_SAMPLES + 1]> = LazyLock::new(|| {
        ...
        let mut x_min = 0f32;
        for (i, sample) in spline_position
            .iter_mut()
            .take(NB_SAMPLES)
            .enumerate()
        {
        ...
        match event.phase {
            ...
            TouchPhase::Moved => {
                match mem::replace(&mut self.state, TouchGestureState::Idle) {
                    ...
                    TouchGestureState::Panning { mut touch, axis }
                        if touch.id == event.id =>
                    {
                        ...
                        if touch.id == event.id =>
                    {
                        ...
                        if speed >= self.tuning.min_fling_velocity {
                            ...
                            if !duration.is_zero() {
                                ...
                                let overshoot = -(f32::from(release_delta.x)
                                    * direction.x
                                    + f32::from(release_delta.y) * direction.y);
                                let emitted_distance =
                                    if overshoot > 0. && overshoot < total_distance {
                                        release_delta += point(
...
mod tests {
    ...
    fn touch_within_slop_resolves_to_tap() {
        ...
        let recognized = recognizer
            .handle_event_at(&touch_event(touch, TouchPhase::Started, 10., 10.), now);
    ...
    fn consecutive_taps_accumulate_tap_count() {
        ...
        recognizer.handle_event_at(
            &touch_event(TouchId(1), TouchPhase::Started, 10., 10.),
            now,
        );
        recognizer.handle_event_at(
    ...
    fn touch_beyond_slop_resolves_to_pan() {
        ...
        recognizer
            .handle_event_at(&touch_event(touch, TouchPhase::Started, 100., 100.), now);
    ...
    fn predicted_positions_lead_the_pan_but_totals_converge_on_release() {
        ...
        let recognized =
            recognizer.handle_event_at(&moved, now + Duration::from_millis(16));
        let [RecognizedTouchGesture::Scroll(scroll)] = recognized.as_slice() else {
            panic!("expected scroll, got {recognized:?}");
        };
        ...
        let recognized =
            recognizer.handle_event_at(&moved, now + Duration::from_millis(32));
        let [RecognizedTouchGesture::Scroll(scroll)] = recognized.as_slice() else {
            panic!("expected scroll, got {recognized:?}");
        };
    ...
    fn predicted_positions_do_not_emit_false_reversals() {
        ...
        let recognized =
            recognizer.handle_event_at(&moved, now + Duration::from_millis(40));
        let [RecognizedTouchGesture::Scroll(scroll)] = recognized.as_slice() else {
            panic!("expected scroll, got {recognized:?}");
        };
        assert_eq!(
        ...
        let recognized =
            recognizer.handle_event_at(&moved, now + Duration::from_millis(48));
        let [RecognizedTouchGesture::Scroll(scroll)] = recognized.as_slice() else {
            panic!("expected scroll, got {recognized:?}");
        };
        assert_eq!(scroll.delta.pixel_delta(px(16.)), point(px(0.), px(6.)));
        ...
        let recognized =
            recognizer.handle_event_at(&moved, now + Duration::from_millis(64));
        let [RecognizedTouchGesture::Scroll(scroll)] = recognized.as_slice() else {
            panic!("expected scroll, got {recognized:?}");
        };
    ...
    fn predicted_overshoot_folds_into_the_fling_without_scrolling_backwards() {
        ...
        for use_prediction in [true, false] {
            ...
            for step in 1..=5u64 {
                ...
                let recognized = recognizer
                    .handle_event_at(&moved, now + Duration::from_millis(step * 16));
    ...
    fn fast_release_starts_momentum_that_decays_to_a_stop() {
        ...
        recognizer
            .handle_event_at(&touch_event(touch, TouchPhase::Started, 100., 300.), now);
        for step in 1..=5 {
            recognizer.handle_event_at(
    ...
    fn spline_position_table_matches_the_bezier_curve() {
        ...
        for (i, &stored_position) in samples.iter().enumerate().take(100) {
            ...
            let (time, position) =
                friction_spline::bezier_time_and_position((lower + upper) / 2.);
            assert!(
                (time - alpha).abs() < 1e-4,
    ...
    fn claimed_touch_drag_emits_phased_stream_without_pan_or_tap() {
        ...
        let now = Instant::now();
        recognizer
            .handle_event_at(&touch_event(touch, TouchPhase::Started, 10., 20.), now);
        let Some(RecognizedTouchGesture::TouchDrag(started)) =
            recognizer.offer_touch_drag(touch)
        else {
    ...
    fn claimed_long_press_emits_phased_stream_without_tap() {
        ...
        recognizer
            .handle_event_at(&touch_event(touch, TouchPhase::Started, 10., 20.), now);
        let Some(RecognizedTouchGesture::LongPress(started)) =
            recognizer.offer_long_press(touch)
        else {
    ...
    fn long_press_cannot_be_offered_after_pending_touch_resolves() {
        for phase in [TouchPhase::Ended, TouchPhase::Cancelled, TouchPhase::Moved] {
            ...
            let now = Instant::now();
            recognizer
                .handle_event_at(&touch_event(touch, TouchPhase::Started, 0., 0.), now);
    ...
    fn claimed_long_press_emits_cancelled_for_its_touch_only() {
        ...
        let cancelled =
            recognizer.handle_event(&touch_event(touch, TouchPhase::Cancelled, 6., 7.));
    ...
    fn completed_touch_id_cannot_claim_replacement_touch() {
        ...
        recognizer.handle_event(&touch_event(
            completed_touch,
            TouchPhase::Started,
            0.,
            0.,
        ...
        assert!(
            recognizer
                .offer_long_press(completed_touch)
                .is_none()
        );
        assert!(
            recognizer
                .offer_long_press(replacement_touch)
                .is_some()
        );
    }
