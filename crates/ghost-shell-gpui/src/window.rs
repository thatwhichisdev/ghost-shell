...
use crate::{
    ...
    Bounds, BoxShadow, Capslock, Context, Corners, CursorHideMode, CursorStyle,
    Decorations, DevicePixels, DispatchActionListener, DispatchNodeId, DispatchTree,
    DisplayId, Edges, Effect, Entity, EntityId, EventEmitter, FileDropEvent, FontId,
    Global, GlobalElementId, GlyphId, GpuSpecs, Hsla, InputHandler, IsZero, KeyBinding,
    KeyContext, KeyDownEvent, KeyEvent, Keystroke, KeystrokeEvent, LayoutId,
    LineLayoutIndex, Modifiers, ModifiersChangedEvent, MonochromeSprite, MouseButton,
    MouseEvent, MouseMoveEvent, MouseUpEvent, Path, Pixels, PlatformAtlas,
    PlatformDisplay, PlatformInput, PlatformInputHandler, PlatformWindow, Point,
    PolychromeSprite, Priority, PromptButton, PromptLevel, Quad, Render,
    RenderGlyphParams, RenderImage, RenderImageParams, RenderSvgParams, Replay,
    ResizeEdge, SMOOTH_SVG_SCALE_FACTOR, SUBPIXEL_VARIANTS_X, SUBPIXEL_VARIANTS_Y,
    ScaledPixels, Scene, Shadow, SharedString, Size, StrikethroughStyle, Style,
    SubpixelSprite, SubscriberSet, Subscription, TabStopMap, TaffyLayoutEngine, Task,
    ...
    WindowDecorations, WindowOptions, WindowParams, WindowTextSystem, WindowVisibility,
...
impl WindowFocusEvent {
    pub fn is_focus_in(&self, focus_id: FocusId) -> bool {
        !self.previous_focus_path.contains(&focus_id)
...
pub(crate) fn with_element_arena<R>(callback: impl FnOnce(&mut Arena) -> R) -> R {
    ...
    let access = with_element_arena_erased(&mut |arena| {
        result = Some(callback
            .take()
            .expect("arena callback runs once")(arena));
    });
    drop(callback);
    access
...
pub(crate) struct Frame {
    ...
    pub(crate) next_inspector_instance_ids:
        FxHashMap<Rc<crate::InspectorElementPath>, usize>,
    #[cfg(any(feature = "inspector", debug_assertions))]
...
impl Frame {
    ...
    pub(crate) fn finish(&mut self, prev_frame: &mut Self) {
        for element_state_key in &self.accessed_element_states {
            if let Some((element_state_key, element_state)) = prev_frame
                .element_states
                .remove_entry(element_state_key)
            {
                self.element_states
                    .insert(element_state_key, element_state);
            }
        }
...
pub struct Window {
    ...
    pub(crate) visibility_observers: SubscriberSet<
        (),
        Box<dyn FnMut(WindowVisibility, &mut Window, &mut App) -> bool>,
    >,
...
impl PendingInputTimeout {
    ...
    fn pause(&mut self, pause: PendingInputTimeoutPause, now: Instant) -> bool {
        match std::mem::replace(
            &mut self.state,
            PendingInputTimeoutState::Paused { pause },
        ) {
            PendingInputTimeoutState::Running { started_at, task } => {
                self.remaining = self
                    .remaining
                    .saturating_sub(now - started_at);
                drop(task);
                true
            }
    ...
    fn resume(
        ...
        match std::mem::replace(
            ...
            PendingInputTimeoutState::Paused { pause } if pause.owner_id == owner_id => {
                true
            }
            previous_state => {
                self.state = previous_state;
    ...
    fn status(&self) -> PendingInputTimeoutStatus {
        let (started_at, paused) = match &self.state {
            PendingInputTimeoutState::Running { started_at, .. } => {
...
impl Window {
    pub(crate) fn new(
        ...
        let display_id = platform_window
            .display()
        ...
        if !accessibility_force_disabled {
            ...
            let (deactivation_sender, deactivation_receiver) =
                async_channel::unbounded::<()>();
        ...
        platform_window.on_request_frame(Box::new({
            ...
            move |request_frame_options| {
                ...
                let force_render = mem::take(&mut deferred_force_render)
...
impl Window {
    ...
    pub(crate) fn refresh_visibility(&mut self, cx: &mut App) {
        ...
        if self.invalidator.is_dirty() || self.needs_present.get() {
            profiler::journal::record_frame_pending(
                self.handle.window_id(),
                Instant::now(),
            );
        }
    ...
    pub fn request_decorations(&self, decorations: WindowDecorations) {
        self.platform_window
            .request_decorations(decorations);
    }
        ...
        AsyncFn: AsyncFnOnce(&mut AsyncWindowContext) -> R + 'static,
    {
        ...
        cx.spawn(async move |app| {
            let mut async_window_cx =
                AsyncWindowContext::new_context(app.clone(), handle);
    ...
    pub fn line_height(&self) -> Pixels {
        self.text_style()
            .line_height_in_pixels(self.rem_size())
    }
    ...
    pub fn is_action_available(&self, action: &dyn Action, cx: &App) -> bool {
        let node_id = self
            .focus_node_id_in_rendered_frame(self.focused(cx).map(|handle| handle.id));
        self.rendered_frame
            .dispatch_tree
    ...
    pub fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        ...
        self.next_frame
            .finish(&mut self.rendered_frame);
    ...
    fn present(&mut self) {
        ...
        self.platform_window
            .draw(&self.rendered_frame.scene);
        #[cfg(feature = "profiler")]
    ...
    fn draw_roots(&mut self, cx: &mut App) {
        ...
        let should_send_a11y_update =
            a11y_active_start_of_frame && a11y_active_end_of_frame;
    ...
    fn prepaint_tooltip(&mut self, cx: &mut App) -> Option<AnyElement> {
        ...
        for tooltip_request_index in (0..self.next_frame.tooltip_requests.len()).rev() {
            ...
            let mut element = tooltip_request
                .tooltip
                .view
                .clone()
                .into_any_element();
            ...
            let tooltip_size =
                element.layout_as_root(AvailableSpace::min_size(), self, cx);
            ...
            let is_visible =
                (tooltip_request
                    .tooltip
                    .check_visible_and_update)(tooltip_bounds, self, cx);
            if !is_visible {
                continue;
            }
        ...
        loop {
            ...
            for deferred_draw_ix in traversal_order {
                ...
                if let Some(mut element) = element {
                    ...
                    self.next_frame.deferred_draws[deferred_draw_ix].element =
                        Some(element);
                } else {
        ...
        self.next_frame.deferred_draws.extend(
            self.rendered_frame.deferred_draws
                ...
                .map(|deferred_draw| DeferredDraw {
                    ...
                    parent_node: reused_subtree
                        .refresh_node_id(deferred_draw.parent_node),
    ...
    pub fn use_asset<A: Asset>(
        ...
        cx.asset_entry::<A>(source)
            .use_by(self.current_view())
    }
        ...
        S: 'static,
    {
        ...
        if let Some(global_id) = global_id {
            self.with_element_state(global_id, |state, cx| {
                ...
                let state = state
                    .expect("you must return some state when you pass some element id");
                (result, state)
            })
        } else {
            let (result, state) = f(None, self);
    ...
    pub fn defer_draw(
        ...
        self.next_frame
            ...
            .push(DeferredDraw {
                ...
                absolute_offset,
                prepaint_range: PrepaintStateIndex::default()
    ...
    pub fn paint_inset_shadows(
        ...
        for shadow in shadows {
            ...
            let hole_corner_radii = Corners {
                ...
                bottom_right: (corner_radii.bottom_right - shadow.spread_radius)
                    .max(zero),
    ...
    fn should_use_subpixel_rendering(&self, font_id: FontId, font_size: Pixels) -> bool {
        if self.platform_window.background_appearance()
    ...
    pub fn paint_svg(
        ...
        let svg_bounds = Bounds {
            origin: bounds.center()
                - Point::new(
                    ScaledPixels(
                        tile.bounds.size.width.0 as f32 / SMOOTH_SVG_SCALE_FACTOR / 2.,
                    ),
                    ScaledPixels(
                        tile.bounds.size.height.0 as f32 / SMOOTH_SVG_SCALE_FACTOR / 2.,
                    ),
                ),
            size: tile
                .bounds
    ...
    pub fn paint_image(
        ...
        if image_bounds.size.width <= Pixels::ZERO
        ...
        let sub_tile = if visible_bounds == image_bounds {
            ...
            let x_offset_ratio = (visible_bounds.origin.x - image_bounds.origin.x)
                / image_bounds.size.width;
            let y_offset_ratio = (visible_bounds.origin.y - image_bounds.origin.y)
            ...
            let sub_origin_x =
                tile_origin_x + (x_offset_ratio * tile_width as f32).round() as i32;
            let sub_origin_y =
                tile_origin_y + (y_offset_ratio * tile_height as f32).round() as i32;
            ...
            let clamped_width = sub_width
                .min(max_x - clamped_origin_x)
                .max(0);
            let clamped_height = sub_height
                .min(max_y - clamped_origin_y)
                .max(0);
    ...
    ///
    /// This method should only be called as part of the paint phase of element drawing.
    pub fn insert_window_control_hitbox(
        &mut self,
    ...
    pub fn on_modifiers_changed(
        ...
        self.next_frame
            ...
            .on_modifiers_changed(Rc::new(
                move |event: &ModifiersChangedEvent,
                      window: &mut Window,
                      cx: &mut App| { listener(event, window, cx) },
            ));
    }
    ...
    pub fn dispatch_event(
        ...
        self.last_input_modality = match &event {
            ...
            PlatformInput::MouseMove(_) | PlatformInput::MouseDown(_) => {
                InputModality::Mouse
            }
        ...
        let event = match event {
            ...
            PlatformInput::FileDrop(file_drop) => match file_drop {
                FileDropEvent::Entered { position, paths } => {
                    ...
                    if !cx.restore_platform_drag(source_window)
    ...
    fn promote_external_drag_to_platform(&mut self, event: &PlatformInput, cx: &mut App) {
        ...
        if Bounds::new(Point::default(), self.viewport_size)
            .contains(&mouse_move.position)
        {
            return;
        }
        if !self
    ...
    fn dispatch_touch_event(&mut self, event: &TouchEvent, cx: &mut App) {
        ...
        if event.phase == crate::TouchPhase::Started
            && let Some(touch_drag) = self
                .touch_gestures
                .offer_touch_drag(event.id)
        {
    ...
    fn start_pending_input_timeout(
        &self,
        remaining: Duration,
        cx: &App,
    ) -> (Instant, Task<()>) {
    ...
    pub fn set_pending_input_timeout_paused<T: 'static>(
        ...
        let release_subscription =
            self.observe_release(owner, cx, move |_, window, cx| {
    ...
    fn focus_node_id_in_rendered_frame(
        &self,
    ...
    fn dispatch_action_on_node_inner(
            ...
            .remove(&action.as_any().type_id())
        {
            for listener in &global_listeners {
                #[cfg(feature = "profiler")]
                self.window_profiler
                    .begin_action_handler(action, cx);
    ...
    pub fn bindings_for_action_in(
        ...
        let dispatch_tree = &self.rendered_frame.dispatch_tree;
        let Some(context_stack) = self.context_stack_for_focus_handle(focus_handle)
        else {
            return vec![];
        };
    ...
    pub fn handler_for<
        E: 'static,
        Callback: Fn(&mut E, &mut Window, &mut Context<E>) + 'static,
    >(
        &self,
        entity: &Entity<E>,
        f: Callback,
        ...
        move |window: &mut Window, cx: &mut App| {
            entity
                .update(cx, |entity, cx| f(entity, window, cx))
                .ok();
        }
    }
    ...
    pub fn on_window_should_close(
        ...
        self.platform_window
            .on_should_close(Box::new(move || {
                cx.update(|window, cx| f(window, cx))
                    .unwrap_or(true)
            }))
    }
    ...
    fn prepaint_inspector(
        ...
        if let Some(inspector) = self.inspector.take() {
            let mut inspector_element =
                AnyView::from(inspector.clone()).into_any_element();
    ...
    fn paint_inspector(
        &mut self,
        mut inspector_element: Option<AnyElement>,
        cx: &mut App,
    ) {
    ...
    fn handle_inspector_mouse_event(&mut self, event: &dyn Any, cx: &mut App) {
            ...
            .is_some()
        {
            ...
            .downcast_ref::<crate::MouseDownEvent>()
            .is_some()
        {
            inspector.update(cx, |inspector, _cx| {
                if let Some((_, inspector_id)) =
    ...
    #[inline(never)]
    fn observe_keyed_state<S: 'static>(
        state: &Entity<S>,
        current_view: EntityId,
        cx: &mut App,
    ) {
...
#[inline(never)]
fn with_element_arena_erased(
    callback: &mut dyn FnMut(&mut Arena),
) -> Result<(), AccessError> {
...
mod tests {
    ...
    use crate::{
        ...
        DragMoveEvent, Empty, ExternalDragPayload, ExternalPaths, FileDragPaths,
        FileDropEvent, FocusHandle, InputEvent as _, InteractiveElement as _,
        IntoElement, KeyDownEvent, Keystroke, LongPressEvent, MouseButton,
        MouseDownEvent, MouseMoveEvent, ParentElement, Pixels, PlatformInput, Point,
        Render, RequestFrameOptions, ScaledPixels, StatefulInteractiveElement as _,
        Styled, TestAppContext, TouchDragEvent, TouchEvent, TouchId, TouchPhase,
        Underline, UnderlineStyle, Window, WindowAppearance, WindowOptions, canvas, div,
        hsla, point, px, size,
    };
    ...
    fn test_window_reports_no_raw_handle_instead_of_panicking(cx: &mut TestAppContext) {
        use raw_window_handle::{
            HandleError, HasDisplayHandle as _, HasWindowHandle as _,
        };
    ...
    impl Render for FileDropExitView {
        fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
            div().size_full().on_file_drop_exit({
                ...
                move |_, _, _| {
                    observed_file_drop_exit.set(observed_file_drop_exit.get() + 1)
                }
            })
        }
    }
            ...
            div()
                ...
                .on_drag_move({
                    ...
                    move |event: &DragMoveEvent<PathBuf>, _, _| {
                        observed_drag_moves
                            .borrow_mut()
                            .push(event.event.position);
                    }
                })
                .on_drop({
                    let observed_drops = self.observed_drops.clone();
                    move |path: &PathBuf, _, _| {
                        observed_drops
                            .borrow_mut()
                            .push(path.clone())
                    }
    ...
    fn file_drag_is_promoted_once_and_restored_in_source_window(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            cx.test_window(successful.window)
                .external_drag_files(),
            ...
            canvas(
                ...
                move |_, _, window, _| {
                    window.on_mouse_event(
                        move |event: &TouchDragEvent, phase, window, _cx| {
                            if phase != DispatchPhase::Bubble {
                                return;
                            }
                            events
                                .borrow_mut()
                                .push((event.phase, event.position.x));
    ...
    fn long_press_is_claimed_only_when_started_prevents_default(cx: &mut TestAppContext) {
        for response in [
            ...
            cx.executor()
                .advance_clock(Duration::from_millis(501));
            cx.executor().run_until_parked();
            window
                .update(cx, |_, window, _| {
                    assert_eq!(
    ...
    fn stale_long_press_timer_cannot_affect_replacement_touch(cx: &mut TestAppContext) {
        ...
        cx.executor()
            .advance_clock(Duration::from_millis(251));
        cx.executor().run_until_parked();
    ...
    fn test_underline_exclusions_preserve_device_geometry(cx: &mut TestAppContext) {
        test_underline_paint_at_scales(cx, |window| {
            ...
            let snap = |quarters: i32| {
                ...
                let rounded = scaled.abs() / divisor
                    + i32::from(2 * (scaled.abs() % divisor) > divisor);
                ...
                for width in [0, 1, 3, 84] {
                    for thickness in [0, 1, 4, 7] {
                        for wavy in [false, true] {
                            ...
                            let origin =
                                point(px(origin as f32 / 4.), px(-origin as f32 / 4.));
                            let width = px(width as f32 / 4.);
                            assert_eq!(
                                window.underline_bounds(origin, width, &style),
                                bounds
                            );
    ...
    #[gpui::test]
    fn test_underline_exclusions_preserve_helvetica_warning_edge(
        cx: &mut TestAppContext,
    ) {
        test_underline_paint_at_scales(cx, |window| {
    ...
    #[gpui::test]
    fn test_underline_exclusions_preserve_content_mask_and_opacity(
        cx: &mut TestAppContext,
    ) {
        test_underline_paint_at_scales(cx, |window| {
            ...
            window.with_content_mask(Some(mask), |window| {
                window.with_element_opacity(Some(0.5), |window| {
                    window.with_element_opacity(Some(0.5), |window| {
                        ...
                        let expected =
                            [(4. * scale, 6. * scale), (8. * scale, 16. * scale)].map(
                                |(left, right)| {
                                    Bounds::from_corners(
                                        point(
                                            ScaledPixels(left),
                                            ScaledPixels((10. * scale).floor()),
                                        ),
                                        point(
                                            ScaledPixels(right),
                                            ScaledPixels((12. * scale).ceil()),
                                        ),
                                    )
                                },
                            );
                        ...
                        for second_end in endpoints {
                            ...
                            let expected = (left..right)
                                .filter(|column| {
                                    !exclusions
                                        .iter()
                                        .any(|span| span.contains(column))
                                })
                                .collect::<Vec<_>>();
                            let exclusions = exclusions.map(|span| {
                                ScaledPixels(span.start as f32)
                                    ..ScaledPixels(span.end as f32)
                            });
                            let underlines = paint_test_underlines(window, |window| {
                                window.paint_underline_with_exclusions(
                                    origin,
...
mod inspector_tests {
    use super::*;
    use crate::{
        DivInspectorState, InspectorElementId, MouseDownEvent, StyleRefinement,
        TestAppContext, div,
    };
    ...
    fn inspector_only_tracks_its_open_window(cx: &mut TestAppContext) {
        ...
        for _ in 0..2 {
            cx.update_window(windows[0].into(), |root, window, cx| {
                let root = root
                    .downcast::<InspectorTestRoot>()
                    .expect("test root");
                let child_widths = root
                    .read(cx)
                    .child
                    .read(cx)
                    .child_widths
                ...
                window.with_inspector_state::<DivInspectorState, _>(
                    ...
                    |state, _| {
                        let state = state
                            .as_mut()
                            .expect("selected div has style state");
                        assert_eq!(
        ...
        fn render(
            ...
            div()
                .on_children_prepainted(move |bounds, _, _| {
                    *child_widths.borrow_mut() = bounds
                        .iter()
