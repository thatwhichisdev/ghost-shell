...
use lyon::tessellation::{
    BuffersBuilder, FillTessellator, FillVertex, StrokeTessellator, StrokeVertex,
    VertexBuffers,
};
...
impl PathBuilder {
    ...
    pub fn translate(&mut self, to: Point<Pixels>) {
        if let Some(transform) = self.transform {
            self.transform =
                Some(transform.then_translate(Vector2D::new(to.x.0, to.y.0)));
        } else {
    ...
    pub fn build(self) -> Result<Path<Pixels>, Error> {
        ...
        match self.style {
            PathStyle::Stroke(options) => {
                Self::tessellate_stroke(self.dash_array, &path, &options)
            }
