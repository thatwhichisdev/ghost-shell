...
impl A11y {
    ...
    pub(crate) fn set_focus(&mut self, node_id: NodeId) {
        ...
        if !self.focus_ids.contains_key(&node_id) {
            if cfg!(debug_assertions) {
                panic!(
                    "set_focus called for a node that was not registered with set_focusable"
                );
            } else {
                log::warn!(
    ...
    pub(crate) fn begin_frame(&mut self) {
        ...
        self.nodes
            .begin_frame(self.window_title.as_ref());
    }
    ...
    pub(crate) fn end_frame(&mut self, frame: debug::FrameDebugInfo) -> TreeUpdate {
        ...
        #[cfg(debug_assertions)]
        self.debug
            .capture_node_info(&self.nodes.node_info);
        update
    }
...
impl<'a> A11ySubtreeBuilder<'a> {
    ...
    pub fn parent_node(&mut self) -> &mut accesskit::Node {
        self.nodes.current_node_mut().expect(
...
impl A11yNodeBuilder {
        ...
        let focus = match self.active_descendant {
            ...
            Some(id) => {
                if cfg!(debug_assertions) {
                    ...
                    log::warn!(
                        "active_descendant set to {id:?}, which is not in the tree"
                    );
    ...
    fn repair_tree_update(mut update: TreeUpdate) -> TreeUpdate {
        let node_ids: FxHashSet<NodeId> = update
            .nodes
            .iter()
            .map(|(id, _)| *id)
            .collect();
