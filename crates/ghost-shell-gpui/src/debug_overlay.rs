...
use crate::{
    BorderStyle, Bounds, ContentMask, Corners, Edges, Hsla, Pixels, Quad, ScaledPixels,
...
impl DebugFrameOverlay {
    ...
    pub(crate) fn paint(
        ...
        let panel_width =
            cell * (max_line_chars as f32 * CHAR_ADVANCE + 2.0 * PANEL_PADDING);
        let panel_height =
            cell * (lines.len() as f32 * LINE_ADVANCE + 2.0 * PANEL_PADDING);
        ...
        for (line_index, line) in lines.iter().enumerate() {
            let line_top =
                panel_top + cell * (PANEL_PADDING + line_index as f32 * LINE_ADVANCE);
            for (char_index, character) in line.chars().enumerate() {
                let Some(rows) = glyph(character) else {
                    continue;
                };
                let glyph_left = panel_left
                    + cell * (PANEL_PADDING + char_index as f32 * CHAR_ADVANCE);
        ...
        match self.mode {
            ...
            DebugFrameOverlayMode::Full => {
                let mut sorted: Vec<Duration> =
                    self.draw_durations.iter().copied().collect();
                sorted.sort_unstable();
                let percentile = |numerator: usize| {
                    (!sorted.is_empty())
                        .then(|| sorted[(sorted.len() - 1) * numerator / 100])
                };
