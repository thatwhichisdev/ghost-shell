...
impl DispatchTree {
    ...
    fn move_node(&mut self, source: &mut DispatchNode) {
        ...
        target.modifiers_changed_listeners =
            mem::take(&mut source.modifiers_changed_listeners);
    }
    ...
    fn binding_matches_predicate_and_not_shadowed(
        ...
        if let Some(found) = bindings.iter().next() {
            found
                .action
                .partial_eq(binding.action.as_ref())
        } else {
            false
        }
    }
    ...
    pub fn dispatch_key(
        ...
        let (bindings, pending, context_stack) =
            self.bindings_for_input(&input, dispatch_path);
    ...
    fn replay_prefix(
        ...
        for last in (0..input.len()).rev() {
            let (bindings, _, _) =
                self.bindings_for_input(&input[0..=last], dispatch_path);
...
mod tests {
    ...
    use crate::{
        ...
        InputHandler, IntoElement, KeyBinding, KeyContext, Keymap, Pixels,
        PlatformWindow, Point, Render, Subscription, TestAppContext, UTF16Selection,
    ...
    fn test_bindings_for_action_hides_targeted_unbind_in_active_context() {
        ...
        assert!(
            fallback_bindings[0]
                .action
                .partial_eq(&SecondaryTestAction)
        );
    }
    ...
    fn test_overlay_after_base_restores_shadowed_picker_binding() {
        ...
        let highest =
            shadowed.highest_precedence_binding_for_action(&TestAction, &contexts);
        assert!(
            highest.is_none(),
        ...
        let highest = override_before_base
            .highest_precedence_binding_for_action(&TestAction, &contexts);
        assert!(
            highest.is_none(),
    ...
    #[crate::test]
    fn test_pending_input_observers_notified_on_focus_change_and_blur(
        cx: &mut TestAppContext,
    ) {
        #[derive(Clone)]
        ...
        let pending_input_changed_count_for_observer =
            pending_input_changed_count.clone();
        ...
        cx.update(|_, _| {
            ...
            assert!(
                count_after_focus_change > *count_after_pending_for_assertion.borrow()
            );
        });
    ...
    fn test_printable_pending_input_replays_on_timeout(cx: &mut TestAppContext) {
        ...
        cx.executor()
            .advance_clock(crate::PENDING_INPUT_TIMEOUT);
        cx.run_until_parked();
    ...
    fn test_pending_input_timeout_dispatches_shorter_binding(cx: &mut TestAppContext) {
        let (cx, action_count, secondary_action_count) =
            setup_pending_input_timeout_test(cx);
    ...
    #[crate::test]
    fn test_running_pending_input_timeout_resets_when_binding_advances(
        cx: &mut TestAppContext,
    ) {
        let (cx, action_count, secondary_action_count) =
            setup_pending_input_timeout_test(cx);
    ...
    #[crate::test]
    fn test_pending_input_timeout_starts_when_binding_becomes_ambiguous(
        cx: &mut TestAppContext,
    ) {
    ...
    #[crate::test]
    fn test_pending_input_timeout_resumes_when_owner_is_released(
        cx: &mut TestAppContext,
    ) {
        let (cx, action_count, secondary_action_count) =
            setup_pending_input_timeout_test(cx);
    ...
    fn test_input_handler_pending(cx: &mut TestAppContext) {
        ...
        let prefers_ime_after_blur = {
            ...
            let prefers_ime = input_handler.as_mut().map(|input_handler| {
