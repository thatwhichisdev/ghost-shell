...
pub fn visibility_style_methods(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        ...
        #visibility fn visible(mut self) -> Self {
            self.style().visibility = Some(ghost_shell_gpui::Visibility::Visible);
            self
        }
        ...
        #visibility fn invisible(mut self) -> Self {
            self.style().visibility = Some(ghost_shell_gpui::Visibility::Hidden);
            self
        }
    };
...
pub fn position_style_methods(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        ...
        #visibility fn relative(mut self) -> Self {
            self.style().position = Some(ghost_shell_gpui::Position::Relative);
            self
        }
        ...
        #visibility fn absolute(mut self) -> Self {
            self.style().position = Some(ghost_shell_gpui::Position::Absolute);
            self
        }
...
pub fn overflow_style_methods(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        ...
        #visibility fn overflow_hidden(mut self) -> Self {
            self.style().overflow.x = Some(ghost_shell_gpui::Overflow::Hidden);
            self.style().overflow.y = Some(ghost_shell_gpui::Overflow::Hidden);
            self
        }
...
pub fn cursor_style_methods(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        ...
        #visibility fn cursor_default(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::Arrow);
            self
        }
        ...
        #visibility fn cursor_pointer(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::PointingHand);
            self
        }
        ...
        #visibility fn cursor_text(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::IBeam);
            self
        }
        ...
        #visibility fn cursor_move(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ClosedHand);
            self
        }
        ...
        #visibility fn cursor_not_allowed(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::OperationNotAllowed);
            self
        }
        ...
        #visibility fn cursor_context_menu(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ContextualMenu);
            self
        }
        ...
        #visibility fn cursor_crosshair(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::Crosshair);
            self
        }
        ...
        #visibility fn cursor_vertical_text(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::IBeamCursorForVerticalLayout);
            self
        }
        ...
        #visibility fn cursor_alias(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::DragLink);
            self
        }
        ...
        #visibility fn cursor_copy(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::DragCopy);
            self
        }
        ...
        #visibility fn cursor_grab(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::OpenHand);
            self
        }
        ...
        #visibility fn cursor_ew_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeLeftRight);
            self
        }
        ...
        #visibility fn cursor_ns_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeUpDown);
            self
        }
        ...
        #visibility fn cursor_nesw_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeUpRightDownLeft);
            self
        }
        ...
        #visibility fn cursor_nwse_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeUpLeftDownRight);
            self
        }
        ...
        #visibility fn cursor_col_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeColumn);
            self
        }
        ...
        #visibility fn cursor_row_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeRow);
            self
        }
        ...
        #visibility fn cursor_n_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeUp);
            self
        }
        ...
        #visibility fn cursor_e_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeRight);
            self
        }
        ...
        #visibility fn cursor_s_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeDown);
            self
        }
        ...
        #visibility fn cursor_w_resize(mut self) -> Self {
            self.style().mouse_cursor = Some(ghost_shell_gpui::CursorStyle::ResizeLeft);
            self
        }
...
pub fn box_shadow_style_methods(input: TokenStream) -> TokenStream {
    ...
    let output = quote! {
        ...
        #visibility fn shadow(mut self, shadows: std::vec::Vec<ghost_shell_gpui::BoxShadow>) -> Self {
        ...
        #visibility fn shadow_2xs(mut self) -> Self {
            use ghost_shell_gpui::{BoxShadow, hsla, px};
            use std::vec;
...
fn generate_methods() -> Vec<TokenStream2> {
    ...
    let mut methods = generate_box_style_methods(
        box_prefixes(),
        box_style_suffixes(),
        visibility.clone(),
    );
...
fn generate_predefined_setter(
    ...
    let field_assignments = fields
        ...
        .map(|field_tokens| {
            quote! {
                style.#field_tokens = Some((#negation_token ghost_shell_gpui::#length_tokens).into());
            }
        })
        .collect::<Vec<_>>();
...
fn generate_custom_value_setter(
    ...
    let method = quote! {
        #[doc = #doc_string]
        #visibility fn #method_name(mut self, length: impl std::clone::Clone + Into<ghost_shell_gpui::#length_type>) -> Self {
            let style = self.style();
            #(#field_assignments)*
            self
        }
