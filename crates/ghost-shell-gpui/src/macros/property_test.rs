...
pub fn test(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Ok(func) = parse2::<ItemFn>(item) else {
        return quote_spanned! { item_span =>
            compile_error!("#[ghost_shell_gpui::property_test] must be placed on a function");
        };
    };
    ...
    let proptest_args = quote! {
        #[strategy = ::ghost_shell_gpui::seed_strategy()] __seed: u64,
        #proptest_args
    };
    ...
    let run_test_body = match &asyncness {
        ...
        Some(_) => quote! {
            let foreground_executor = ghost_shell_gpui::ForegroundExecutor::new(std::sync::Arc::new(dispatcher.clone()));
            #cx_vars
    ...
    quote! {
        ...
        fn #test_name(#proptest_args) #test_ret_ty {
            ...
            ::ghost_shell_gpui::run_test_once(
                __seed,
...
impl Args {
    ...
    /// `config = ...` tokens and add a call to `ghost_shell_gpui::apply_seed_to_config`.
    fn render(&self) -> TokenStream {
        let user_provided_config = match &self.config {
            None => {
                quote! { ::ghost_shell_gpui::proptest::prelude::ProptestConfig::default() }
            }
            Some(config) => config.into_token_stream(),
        };

        let fixed_config = quote!(::ghost_shell_gpui::apply_seed_to_proptest_config(#user_provided_config));
        let remaining_args = &self.remaining_args;
        let errors = &self.errors;

        quote! {
            #errors
            #[::ghost_shell_gpui::proptest::property_test(
                proptest_path = "::ghost_shell_gpui::proptest",
                config = #fixed_config,
                #(#remaining_args,)*
            )]
        }
        ...
        for pair in pairs {
            match pair.path.get_ident().map(Ident::to_string).as_deref() {
                ...
                Some("proptest_path") => errors.extend(quote_spanned! {pair.span() =>
                    compile_error!("`ghost_shell_gpui::property_test` overrides the `proptest_path` parameter")
                }),
...
fn remove_background_executor(parsed: &mut ParsedArgs, args: &mut Vec<FnArg>) {
    args.retain_mut(|arg| {
        ...
        parsed
            .inner_fn_args
            .extend(quote!(ghost_shell_gpui::BackgroundExecutor::new(
