use std::{
    ...
    iter::FusedIterator,
    sync::{
        Arc, Condvar, Mutex, MutexGuard, PoisonError, TryLockError, atomic::AtomicUsize,
    },
};
...
impl<T> PriorityQueueState<T> {
    ...
    fn try_recv<'a>(
        &'a self,
    ) -> Result<Option<MutexGuard<'a, PriorityQueues<T>>>, RecvError> {
        let queues = self
            .queues
            .lock()
            .unwrap_or_else(PoisonError::into_inner);
...
impl<T> PriorityQueueReceiver<T> {
    ...
    pub(crate) fn len(&self) -> usize {
        let queues = self
            ...
            .lock()
            .unwrap_or_else(PoisonError::into_inner);
        queues.high_priority.len()
            + queues.medium_priority.len()
            + queues.low_priority.len()
    }
    ...
    pub fn spin_try_pop(&mut self) -> Result<Option<T>, RecvError> {
        ...
        if !queues.high_priority.is_empty() {
            let flip = self
                .rand
                .random_ratio(P::High.weight(), mass);
            if flip {
        ...
        if !queues.medium_priority.is_empty() {
            let flip = self
                .rand
                .random_ratio(P::Medium.weight(), mass);
            if flip {
