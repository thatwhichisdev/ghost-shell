...
impl Clone for OwnedMenuItem {
    fn clone(&self) -> Self {
        match self {
            ...
            OwnedMenuItem::SystemMenu(os_menu) => {
                OwnedMenuItem::SystemMenu(os_menu.clone())
            }
        }
    }
}
...
pub(crate) fn init_app_menus(platform: &dyn Platform, cx: &App) {
    platform.on_will_open_app_menu(Box::new({
        ...
        move || {
            if let Some(app) = cx.app.upgrade() {
                app.borrow_mut()
                    .update(|cx| cx.clear_pending_keystrokes());
            }
        }
    }));
    ...
    platform.on_validate_app_menu_command(Box::new({
        ...
        move |action| {
            cx.app
                ...
                .map(|app| {
                    app.borrow_mut()
                        .update(|cx| cx.is_action_available(action))
                })
                .unwrap_or(false)
        }
    }));
    ...
    platform.on_app_menu_action(Box::new({
        ...
        move |action| {
            if let Some(app) = cx.app.upgrade() {
                app.borrow_mut()
                    .update(|cx| cx.dispatch_action(action));
            }
        }
    }));
}
