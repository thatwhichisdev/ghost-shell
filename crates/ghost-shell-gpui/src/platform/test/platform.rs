...
use crate::{
    ...
    PlatformDisplay, PlatformHeadlessRenderer, PlatformKeyboardLayout,
    PlatformKeyboardMapper, PlatformTextSystem, PromptButton, SharedString,
    SystemNotification, SystemNotificationResponse, Task, TestDisplay, TestWindow,
...
pub(crate) struct TestPlatform {
    ...
    headless_renderer_factory:
        Option<Box<dyn Fn() -> Option<Box<dyn PlatformHeadlessRenderer>>>>,
    weak: Weak<Self>,
...
impl Platform for TestPlatform {
    ...
    fn prevent_idle_sleep(&self, reason: &str) -> Task<Result<ActivityGuard>> {
        ...
        let acquire = move || {
            if fails {
                anyhow::bail!(
                    "Idle sleep prevention for {reason:?} is set to fail in this test"
                );
            }
    ...
    fn unlock_session(&self) -> Result<()> {
        anyhow::bail!("session locking requires a Wayland compositor")
    }
}
