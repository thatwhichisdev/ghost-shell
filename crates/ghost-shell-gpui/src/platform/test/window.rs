...
use crate::{
    AnyWindowHandle, Bounds, DevicePixels, DispatchEventResult, GpuSpecs, HeadlessAtlas,
    Pixels, PlatformAtlas, PlatformDisplay, PlatformHeadlessRenderer, PlatformInput,
    PlatformInputHandler, PlatformWindow, Point, PromptButton, RequestFrameOptions,
    Scene, Size, TestPlatform, TextInputConfiguration, TextInputStateChange,
...
pub(crate) struct TestWindowState {
    ...
    hit_test_window_control_callback:
        Option<Box<dyn FnMut() -> Option<WindowControlArea>>>,
...
impl PlatformWindow for TestWindow {
    ...
    fn on_input(
        &self,
        callback: Box<dyn FnMut(crate::PlatformInput) -> DispatchEventResult>,
    ) {
        self.0.lock().input_callback = Some(callback)
    }
    ...
    fn on_hit_test_window_control(
        ...
        self.0
            .lock()
            .hit_test_window_control_callback = Some(callback);
    }
    ...
    fn draw(&self, scene: &Scene) {
        ...
        state.frame_scheduled = true;
        let device_size: Size<DevicePixels> = state
            .bounds
            .size
    ...
    fn start_external_drag(&self, payload: &crate::ExternalDragPayload) -> bool {
        ...
        match payload {
            crate::ExternalDragPayload::Files(paths) => {
                state
                    .external_drag_files
                    .extend_from_slice(paths.entries());
            }
        }
