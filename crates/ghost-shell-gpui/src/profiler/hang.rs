...
use super::journal::{
    ForegroundEvent, ForegroundJournal, ForegroundJournalCollector,
    ForegroundJournalEntry, FrameSnapshot, IntervalBoundary, IntervalSealer,
};
...
impl SerializedHangIncident {
    ...
    pub fn convert(
        ...
        Self {
            ...
            contributors_elided: incident
                .contributors
                .len()
                .saturating_sub(max_contributors),
        }
    }
}
...
impl HangIncident {
    ...
    pub fn detect(
        ...
        let trigger = if contributors.is_empty() {
            ...
            let spend = snapshot
                .occupancy_within(snapshot.interval_start, snapshot.interval_end());
...
mod tests {
    ...
    use super::super::journal::{
        ...
        SmallPollFlush, install_test_foreground_journal, record_present,
    };
    use super::{
        HangDetector, HangIncident, HangTrigger, SerializedHangContributor,
    ...
    fn serialized_incident_reports_presented_seal_fields() {
        ...
        let incident = HangIncident::detect(snapshot, HANG_THRESHOLD, FRAME_BUDGET)
            .expect("has contributors");
        let serialized =
            SerializedHangIncident::convert(startup, &incident, 1, Some(at(50)));
    ...
    fn serialized_incident_reports_idle_seal_fields() {
        ...
        let incident = HangIncident::detect(idle, HANG_THRESHOLD, FRAME_BUDGET)
            .expect("has contributors");
    ...
    fn an_interval_of_small_work_over_budget_is_an_incident() {
        ...
        let serialized =
            SerializedHangIncident::convert(startup, &incident, 8, Some(startup));
    ...
    fn a_journal_gap_suppresses_budget_inference_but_retains_observed_hangs() {
        ...
        assert!(
            HangIncident::detect(budget_only.clone(), HANG_THRESHOLD, FRAME_BUDGET)
                .is_none()
        );
        ...
        assert!(
            HangIncident::detect(clean.clone(), HANG_THRESHOLD, FRAME_BUDGET).is_some()
        );
    }
    ...
    fn detects_randomly_placed_foreground_hangs(
        mut rng: StdRng,
        ...
        for _ in 0..rng.random_range(1..=4) {
            let duration =
                HANG_THRESHOLD + Duration::from_millis(rng.random_range(5..25));
        ...
        for kind in [
            ...
            let observed: Vec<Duration> = contributors
                .iter()
                .filter(|event| {
                    matches_kind(event, kind) && event.duration() >= HANG_THRESHOLD
                })
