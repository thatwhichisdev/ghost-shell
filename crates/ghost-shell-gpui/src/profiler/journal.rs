...
const MAX_JOURNAL_ENTRIES: usize =
    (4 * 1024 * 1024) / core::mem::size_of::<JournalSlot>();
...
impl PresentedFrame {
    ...
    pub fn dirty_to_present_duration(&self) -> Option<Duration> {
        self.frame.dirty_at.map(|dirty_at| {
            self.presentation
                .present_end
                .duration_since(dirty_at)
        })
    }
}
            ...
            .map(|flush| {
                ...
                if span.is_zero() {
                    ...
                    flush
                        .summary
                        .total
                        .mul_f64(overlap.div_duration_f64(span))
                }
            })
            .sum();
...
pub(crate) fn install_test_foreground_journal(
    ...
    let previous = FOREGROUND_JOURNAL.with(|journal| {
        journal
            .borrow_mut()
            .replace(ForegroundJournalWriter::new(
                foreground_runnables,
                publisher,
            ))
    });
...
impl JournalPublisher {
    ...
    fn flush_pending(&mut self) {
        while let Some(pending) = self.pending.front() {
            if !self
                .ring
                .try_publish(pending.sequence, pending.entry)
            {
                return;
            }
...
impl ForegroundJournalCollector {
    ...
    pub fn collect_unseen(&mut self) -> DrainedEntries {
        ...
        let mut entries =
            Vec::with_capacity((end - self.cursor) as usize + usize::from(lost > 0));
        if lost > 0 {
        ...
        for entry in entries {
            match entry {
                ...
                ForegroundJournalEntry::Boundary(boundary) => {
                    ...
                    if self.is_empty() {
                        self.interval_start =
                            self.interval_start.max(boundary.end_time());
                    } else {
...
mod tests {
            ...
            collector
                ...
                .all(|entry| matches!(
                    ...
                    ForegroundJournalEntry::Boundary(
                        IntervalBoundary::PowerTransition { .. }
                    )
                ))
        );
    ...
    fn an_immediately_ready_runnable_prevents_an_idle_boundary_between_polls() {
        ...
        let first = ForegroundEvent::TaskPoll(task_timing(
            start,
    ...
    fn a_pending_frame_prevents_idle_until_presentation() {
        ...
        let presentation =
            presentation_timing(window_id, start + Duration::from_millis(3));
    ...
    fn small_polls_are_flushed_immediately_before_a_retained_event() {
        ...
        let Some(ForegroundJournalEntry::Event(ForegroundEvent::SmallPolls(flush))) =
            input_index
                .checked_sub(1)
    ...
    fn sparse_small_polls_are_discarded_when_the_foreground_returns_to_idle() {
        ...
        let mut tiny_wake = |ended_at: Instant| {
            journal.begin_turn();
            journal.fold_small_poll(task_timing(
                ended_at - Duration::from_micros(50),
                ended_at,
            ));
    ...
    fn interval_event_cap_counts_overflowing_events() {
        ...
        let mut sealer = IntervalSealer::new(start);
        let overflow = 5;
        let mut entries: Vec<ForegroundJournalEntry> =
            (0..(MAX_INTERVAL_EVENTS + overflow) as u64)
                .map(|i| {
                    input_entry(
        ...
        specifications
            ...
            .map(|(advance, span, kind)| {
                ...
                let started_at_micros =
                    completed_at_micros.saturating_sub(u64::from(*span));
                ...
                match kind % 7 {
                    ...
                    1 => ForegroundJournalEntry::Event(ForegroundEvent::Action(
                        ActionTiming {
                            name: "test.action",
                            start,
                            end,
                    ...
                    2 => ForegroundJournalEntry::Event(ForegroundEvent::TaskPoll(
                        task_timing(start, end),
                    )),
                    3 => ForegroundJournalEntry::Event(ForegroundEvent::SmallPolls(
                        SmallPollFlush {
                            summary: PollSummary {
                    ...
                    5 => ForegroundJournalEntry::Boundary(IntervalBoundary::Idle {
                        ended_at: end,
                    }),
    ...
    fn normalize_small_polls(
        origin: Instant,
        flush: SmallPollFlush,
                ...
                ForegroundJournalEntry::Boundary(boundary) => {
                    if let IntervalBoundary::Presented(presented) = boundary {
                        ...
                        if events.is_empty()
                            ...
                            && !journal_discontinuous
                        {
                            interval_start = interval_start.max(
                                present
                                    .start_time()
                                    .duration_since(origin)
                                    .as_micros() as u64,
                        ...
                        && !journal_discontinuous
                    {
                        ...
                        snapshots.push(NormalizedSnapshot {
                            ...
                            journal_discontinuous: std::mem::take(
                                &mut journal_discontinuous,
                            ),
                        });
    ...
    impl ModelWriter {
        fn record_entry(&mut self, entry: WriterEntry) {
            ...
            let boundary =
                matches!(entry, WriterEntry::Presented { .. } | WriterEntry::Idle(_));
    ...
    fn normalize_writer_entries(
        ...
        entries
            ...
            .map(|entry| match entry {
                ...
                ForegroundJournalEntry::Boundary(IntervalBoundary::Presented(
                    presented,
