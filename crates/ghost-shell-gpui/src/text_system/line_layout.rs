...
impl LineLayout {
    ...
    pub fn split_at(&self, byte_index: usize) -> (LineLayout, LineLayout) {
        ...
        for run in &self.runs {
            let split_pos = run
                .glyphs
                .partition_point(|g| g.index < byte_index);
...
impl WrappedLineLayout {
    ...
    fn _index_for_position(
        ...
        if wrapped_line_ix > 0 {
            let Some(line_start_boundary) = self.wrap_boundaries.get(wrapped_line_ix - 1)
            else {
                return Err(0);
            };
    ...
    pub fn position_for_index(
        ...
        for (ix, line_end_ix) in line_end_indices {
            ...
            if index < line_start_ix {
                ...
                let line_start_x = self
                    .unwrapped_layout
                    .x_for_index(line_start_ix);
...
impl LineLayoutCache {
    ...
    pub fn reuse_layouts(&self, range: Range<LineLayoutIndex>) {
        ...
        for key in
            &previous_frame.used_lines[range.start.lines_index..range.end.lines_index]
        {
        ...
        {
            if let Some((key, line)) = previous_frame
                .wrapped_lines
                .remove_entry(key)
            {
        ...
        for key in &previous_frame.used_wrapped_lines_by_hash[range
            .start
    ...
    pub fn layout_line_by_hash(
        ...
        if let Some(existing_key) = previous_frame
            ...
            .cloned()
        {
            if let Some((key, layout)) = previous_frame
                .lines_by_hash
                .remove_entry(&existing_key)
            {
                current_frame
                    .lines_by_hash
                    .insert(key.clone(), layout.clone());
...
impl PartialEq for CacheKey {
    fn eq(&self, other: &Self) -> bool {
        self.as_cache_key_ref()
            .eq(&other.as_cache_key_ref())
    }
}
...
mod tests {
    ...
    fn test_force_width_latin_unchanged() {
        let cell_width = px(8.);
        let mut layout =
            make_layout(vec![glyph_at(0., 0), glyph_at(8., 1), glyph_at(16., 2)]);
    ...
    fn test_force_width_base_after_combining_mark() {
        let cell_width = px(8.);
        let mut layout =
            make_layout(vec![glyph_at(0., 0), glyph_at(0., 3), glyph_at(8., 6)]);
