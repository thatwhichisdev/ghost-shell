...
impl LineWrapper {
        ...
        iter::from_fn(move || {
            for candidate in candidates.by_ref() {
                ...
                if width > wrap_width && ix > last_wrap_ix {
                    if let (None, Some(first_non_whitespace_ix)) =
                        (indent, first_non_whitespace_ix)
                    {
                        indent = Some(
                            Self::MAX_INDENT
                                .min((first_non_whitespace_ix - last_wrap_ix) as u32),
                        );
                    }
    ...
    pub fn truncate_line<'a>(
        ...
        if let Some(truncate_ix) = self.should_truncate_line(
            &line,
            truncate_width,
            truncation_affix,
            truncate_from,
            ...
            let result = match truncate_from {
                ...
                TruncateFrom::End => SharedString::from(format!(
                    "{}{truncation_affix}",
                    line[..truncate_ix].trim_end_matches(
                        |c: char| c.is_whitespace() || c.is_ascii_punctuation()
                    )
                )),
                TruncateFrom::Middle => {
                    unreachable!("Middle truncation is handled above")
                }
            };
            let mut runs = runs.to_vec();
        ...
        for (ix, c) in text.char_indices() {
            if c == '\n' {
                if line >= max_lines - 1 && !text[ix + 1..].trim().is_empty() {
                    ...
                    // below. Truncate here.
                    let truncated = text[..truncate_ix].trim_end_matches(|c: char| {
                        c.is_whitespace() || c.is_ascii_punctuation()
                    });
                    let result =
                        SharedString::from(format!("{truncated}{truncation_affix}"));
                    let mut runs = runs.to_vec();
                    update_runs_after_truncation(
                        &result,
                        truncation_affix,
            ...
            if line < max_lines - 1 {
                ...
                if width > wrap_width && ix > last_wrap_ix {
                    if let (None, Some(first_nw)) = (indent, first_non_whitespace_ix) {
                        indent =
                            Some(Self::MAX_INDENT.min((first_nw - last_wrap_ix) as u32));
                    }
...
fn update_runs_after_truncation(
    ...
    match truncate_from {
        ...
        TruncateFrom::Middle => {
            unreachable!(
                "Middle truncation calls this function with TruncateFrom::End directly"
            )
        }
    }
}
...
mod tests {
    use super::*;
    use crate::{
        Font, FontFeatures, FontStyle, FontWeight, TestAppContext, TestDispatcher, font,
    };
    ...
    fn test_is_word_char() {
        ...
        fn assert_not_word(word: &str) {
            let found = word
                .chars()
                .any(|c| !LineWrapper::is_word_char(c));
    ...
    fn test_multiline_truncation_with_newlines() {
        ...
        let second_line_width: Pixels = parts[1]
            .chars()
    ...
    fn test_truncate_line_middle() {
        ...
        let (result, _result_runs) = wrapper.truncate_line(
            long_text.into(),
            px(100.),
            "…",
            &runs,
