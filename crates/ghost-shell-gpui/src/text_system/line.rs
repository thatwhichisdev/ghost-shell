...
use crate::{
    App, Bounds, DevicePixels, Half, Hsla, LineLayout, Pixels, Point, RenderGlyphParams,
    Result, SharedString, StrikethroughStyle, TextAlign, UnderlineStyle, Window,
...
impl<'a> ShapedLineCursor<'a> {
    ...
    pub fn take_until(&mut self, byte_index: usize) -> ShapedLine {
        ...
        while let Some(decoration) = self
            .line
            .decoration_runs
            .get(self.decoration_index)
...
impl LineLayout {
    ...
    pub fn paint(
        ...
        paint_line(
            ...
            window,
            cx,
            &mut |_, origin, width, style, window| {
                window.paint_underline(origin, width, style)
            },
        )
    }
...
fn paint_line(
    ...
    window.paint_layer(line_bounds, |window| {
        ...
        let underline_y_offset =
            underline_y_offset(line_height, layout.ascent, layout.descent);
        let mut decoration_runs = decoration_runs.iter();
        let mut wraps = wrap_boundaries.iter().peekable();
        let mut run_end = 0;
        let mut color = black();
        let mut current_underline: Option<(Point<Pixels>, UnderlineStyle, Range<usize>)> =
            None;
        ...
        for (run_ix, run) in layout.runs.iter().enumerate() {
            ...
            for (glyph_ix, glyph) in run.glyphs.iter().enumerate() {
                ...
                let mut finished_underline: Option<(
                    Point<Pixels>,
                    UnderlineStyle,
                    Range<usize>,
                )> = None;
                let mut finished_strikethrough: Option<(
                    Point<Pixels>,
                    StrikethroughStyle,
                )> = None;
                if glyph.index >= run_end {
                    ...
                    if let Some(style_run) = style_run {
                        ...
                        if let Some((_, underline_style, underline_range)) =
                            &mut current_underline
                        {
                            if style_run.underline.as_ref() != Some(underline_style) {
                                ...
                                underline_range.end =
                                    style_run_start + style_run.len as usize;
                            }
                        }
                        if let Some(run_underline) = style_run.underline.as_ref() {
                            current_underline.get_or_insert((
                                point(
                                    glyph_origin.x,
                                    glyph_origin.y + underline_y_offset,
                                ),
                                UnderlineStyle {
                                    color: Some(
                                        run_underline
                        ...
                        if let Some((_, strikethrough_style)) = &mut current_strikethrough
                            && style_run.strikethrough.as_ref()
                        ...
                        if let Some(run_strikethrough) = style_run.strikethrough.as_ref()
                        {
                            current_strikethrough.get_or_insert((
                                point(
                                    glyph_origin.x,
                                    glyph_origin.y
                                        + (((layout.ascent * 0.5) + baseline_offset.y)
                                            * 0.5),
                                ),
                                StrikethroughStyle {
                                    color: Some(
    ...
    window.paint_layer(line_bounds, |window| {
        ...
        for (run_ix, run) in layout.runs.iter().enumerate() {
            ...
            for (glyph_ix, glyph) in run.glyphs.iter().enumerate() {
                ...
                if glyph.index >= run_end {
                    ...
                    if let Some(style_run) = style_run {
                        if let Some((_, background_color)) = &mut current_background
                            && style_run.background_color.as_ref()
                ...
                if let Some((mut background_origin, background_color)) =
                    finished_background
                {
...
fn aligned_origin_x(
    ...
    let end_of_line = if let Some(WrapBoundary { run_ix, glyph_ix }) = wrap_boundary {
        layout.runs[*run_ix].glyphs[*glyph_ix]
            .position
            .x
    } else {
        layout.width
...
mod tests {
    ...
    use super::*;
    use crate::{
        AppContext as _, Context, FontId, GlyphId, IntoElement, Render, ShapedGlyph,
        ShapedRun, Styled, TestAppContext, TextRun, Underline, canvas, font, hsla,
    };
    ...
    fn test_underline_handler_matches_wrapped_paint(cx: &mut TestAppContext) {
        test_underline_handler_at_scales(cx, |window, cx| {
            ...
            for zero_advance in [false, true] {
                ...
                let line_height = px(20.);
                let y =
                    origin.y + underline_y_offset(line_height, line.ascent, line.descent);
        ...
        for first in 0..=line.len() {
            for second in first..=line.len() {
                ...
                for boundary in [first, second, line.len(), line.len()] {
                    let (expected, rest) =
                        remainder.split_at(boundary - previous_boundary);
                    let actual = cursor.take_until(boundary);
                    assert_eq!(actual.text, expected.text);
                    assert_eq!(actual.len(), expected.len());
                    assert_eq!(actual.width(), expected.width());
                    ...
                    for (actual, expected) in actual.runs.iter().zip(&expected.runs) {
                        ...
                        for (actual, expected) in
                            actual.glyphs.iter().zip(&expected.glyphs)
                        {
                            assert_eq!(actual.id, expected.id);
                            assert_eq!(actual.index, expected.index);
                            assert_eq!(actual.position, expected.position);
                    ...
                    assert_eq!(
                        actual.decoration_runs.len(),
                        expected.decoration_runs.len()
                    );
                    for (actual, expected) in actual
                        .decoration_runs
                        .iter()
                        .zip(&expected.decoration_runs)
                    {
                        assert_eq!(actual.len, expected.len);
                        assert_eq!(actual.color, expected.color);
