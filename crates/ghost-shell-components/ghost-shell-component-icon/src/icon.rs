...
use ghost_shell_gpui::{
    AnyElement, App, AppContext, Context, Entity, Hsla, IntoElement, Pixels, Radians,
    Render, RenderOnce, SharedString, StyleRefinement, Styled, Svg, Transformation,
    Window, prelude::FluentBuilder as _, svg,
};
...
impl From<IconName> for Icon {
    fn from(name: IconName) -> Self {
        let bytes: &[u8] = match name {
            IconName::LoaderCircle => include_bytes!("../assets/loader-circle.svg"),
            IconName::Check => include_bytes!("../assets/check.svg"),
            IconName::ChevronRight => include_bytes!("../assets/chevron-right.svg"),
            IconName::ChevronLeft => include_bytes!("../assets/chevron-left.svg"),
            IconName::ExternalLink => include_bytes!("../assets/external-link.svg"),
        };
...
impl Icon {
    ...
    pub fn transform(mut self, transformation: ghost_shell_gpui::Transformation) -> Self {
    ...
    fn into_svg(self, text_size: Pixels, fallback_color: Hsla) -> Svg {
        ...
        let has_base_size =
            self.style.size.width.is_some() || self.style.size.height.is_some();
...
#[cfg(test)]
mod tests {
    use ghost_shell_gpui::{px, size};

    use super::*;

    const SVG: &[u8] = include_bytes!("../assets/chevron-right.svg");

    #[test]
    fn test_icon_builder_preserves_owned_data_and_transform_on_clone() {
        let transformation = Transformation::scale(size(0.5, 0.5))
            .with_rotation(ghost_shell_gpui::radians(std::f32::consts::FRAC_PI_2));
        let icon = {
            let bytes = SVG.to_vec();
            Icon::default()
                .data(&bytes)
                .large()
                .text_color(ghost_shell_gpui::red())
                .transform(transformation)
        };
        let cloned = icon.clone();
        let (IconSource::Data(original), IconSource::Data(copy)) =
            (&icon.source, &cloned.source)
        else {
        ...
        let mut svg = cloned.into_svg(px(12.), ghost_shell_gpui::blue());
        assert_eq!(svg.style().text.color, Some(ghost_shell_gpui::red()));
        assert_eq!(
            svg.style().size.width,
            Some(ghost_shell_gpui::rems(1.5).into())
        );

        let rotated = icon
            .rotate(ghost_shell_gpui::radians(std::f32::consts::PI))
            .clone();
        assert_eq!(
            rotated.transformation,
            Some(Transformation::rotate(ghost_shell_gpui::radians(
                std::f32::consts::PI
            )))
        );
    }
    ...
    fn test_icon_source_builders_replace_previous_source() {
        let icon = Icon::new(IconName::LoaderCircle).data(SVG);
        assert!(
            matches!(icon.source_ref(), IconSource::Data(bytes) if bytes.as_ref() == SVG)
        );
