...
//!
//! Inspired by `ghost_shell_gpui::uniform_list`.
...
use ghost_shell_gpui::{
    ...
    InteractiveElement, IntoElement, IsZero as _, ListSizingBehavior, Pixels, Point,
    Render, ScrollHandle, ScrollStrategy, Size, Stateful, StatefulInteractiveElement,
...
impl VirtualList {
    ...
    fn scroll_to_deferred_item(
        ...
        let Some(item_index) = scroll_to_item
            .item_index
            .checked_add(scroll_to_item.offset)
        else {
            return scroll_offset;
        };
        let Some(bounds) = size_layout.item_bounds(item_index, self.axis, content_bounds)
        else {
            return scroll_offset;
        };
        ...
        match scroll_to_item.strategy {
            ScrollStrategy::Center => {
                if self.axis == Axis::Vertical {
                    scroll_offset.y = content_bounds.top()
                        + content_bounds.size.height.half()
                        - bounds.top()
                        - bounds.size.height.half()
                } else {
                    scroll_offset.x = content_bounds.left()
                        + content_bounds.size.width.half()
                        - bounds.left()
...
impl ItemSizeLayout {
    ...
    fn item_bounds(
        ...
        let item_size = *self.sizes.get(ix)?;
        Some(match axis {
...
impl Element for VirtualList {
    ...
    fn request_layout(
        ...
        inspector_id: Option<&ghost_shell_gpui::InspectorElementId>,
        window: &mut Window,
        cx: &mut App,
    ) -> (ghost_shell_gpui::LayoutId, Self::RequestLayoutState) {
        ...
        let list_width = if self.axis == Axis::Vertical {
            self.scroll_handle
                .state
                .borrow()
                .last_content_size
        ...
        let layout_id = self.base.interactivity().request_layout(
            ...
            cx,
            |style, window, cx| {
                size_layout = window.with_optional_element_state(
                    global_id,
                    |state: Option<Option<ItemSizeLayout>>, _window| {
                            ...
                            || state.axis != Some(self.axis)
                        {
                            ...
                            state.gap = gap;
                            state.axis = Some(self.axis);
                            state.content_size = Size::default();
                        ...
                        (state.clone(), global_id.map(|_| state))
                    },
                );
                            ...
                            let item_origin = match self.axis {
                                ...
                                Axis::Vertical => {
                                    content_bounds.origin
                                        + point(
                                            scroll_offset.x,
                                            item_origins[ix] + scroll_offset.y,
                                        )
                                }
                            };
...
#[cfg(test)]
mod tests {
    use ghost_shell_gpui::{Context, TestAppContext};
    ...
    fn exercise_axis(cx: &mut TestAppContext, axis: Axis) {
        ...
        let initial_range = visible_ranges
            .borrow()
            .last()
            .cloned()
            .unwrap();
        ...
        cx.update(|window, cx| window.draw(cx).clear(cx));
        let scrolled_range = visible_ranges
            .borrow()
            .last()
            .cloned()
            .unwrap();
    ...
    #[ghost_shell_gpui::test]
    fn horizontal_visible_range_and_deferred_scroll_are_preserved(
