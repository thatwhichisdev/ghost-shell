...
use ghost_shell_theme::{Sizable, Size};
...
#[cfg(test)]
mod tests {
    use ghost_shell_gpui::{Render, TestAppContext, px, size};
    ...
    impl Render for SpinnerHost {
        fn render(
            &mut self,
            _: &mut Window,
            _: &mut ghost_shell_gpui::Context<Self>,
        ) -> impl IntoElement {
            Spinner::new()
        }
    }
    ...
    fn reduced_motion_spinner_is_static_and_requests_no_frame(cx: &mut TestAppContext) {
        ...
        let frames = window.update(cx, |_, window, cx| window.simulate_next_frame(cx));
        assert!(matches!(frames, Ok(0)));
    }
}
