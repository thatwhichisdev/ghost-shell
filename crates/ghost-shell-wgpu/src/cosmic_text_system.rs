...
impl CosmicTextSystemState {
    ...
    fn prewarm_fonts(&mut self, font_ids: &[FontId]) {
        for &font_id in font_ids {
            ...
            self.font_system
                .get_font_matches(&primary_attributes);

            for (fallback_id, fallback_name) in &*properties.fallback_chain {
                let fallback_attributes =
                    properties.attributes(*fallback_id, fallback_name);
                self.font_system
                    .get_font_matches(&fallback_attributes);
            }
        }
    }
    ...
    fn load_family(
        ...
        let user_fallback_chain: Arc<[(FontId, SharedString)]> = match fallbacks {
            Some(fallbacks) if !fallbacks.fallback_list().is_empty() => {
                ...
                for fallback_name in fallbacks.fallback_list() {
                    ...
                    let fb_ids = if let Some(cached) =
                        self.font_ids_by_family_cache.get(&fb_key)
                    {
                        cached.clone()
                    } else {
        ...
        for (database_id, postscript_name) in families {
            ...
            if let Some(&font_id) = self.loaded_font_ids_by_key.get(&key) {
                self.loaded_fonts[font_id.0].user_fallback_chain =
    ...
    fn raster_bounds(
        ...
        if !bounds.is_zero() {
            self.pending_glyph_images
                .insert(params.clone(), image);
        }
        Ok(bounds)
    }
    ...
    fn layout_line_no_separators(
        ...
        for run in font_runs {
            ...
            let fallback_attrs: SmallVec<[Attrs<'_>; 4]> = properties
                ...
                .map(|(font_id, family_name)| {
                    properties.attributes(*font_id, family_name)
                })
                .collect();
        ...
        if let Some((sink, missing_glyphs)) = self
            .missing_glyph_sink
            .as_ref()
            .zip(missing_glyphs)
        {
    ...
    fn missing_glyphs(
        ...
        let mut font_run_end = font_runs
            .first()
            .map_or(0, |font_run| font_run.len);
...
fn find_best_match(
    ...
    for (index, font_id) in candidates.iter().enumerate() {
        ...
        let weight_diff =
            (face_info.weight.0 as i32 - target_weight as i32).unsigned_abs();
...
fn face_info_into_properties(
    ...
    font_kit::properties::Properties {
        ...
        stretch: match face_info.stretch {
            ...
            cosmic_text::Stretch::ExtraCondensed => {
                font_kit::properties::Stretch::EXTRA_CONDENSED
            }
            cosmic_text::Stretch::ExtraExpanded => {
                font_kit::properties::Stretch::EXTRA_EXPANDED
            }
            cosmic_text::Stretch::Normal => font_kit::properties::Stretch::NORMAL,
            cosmic_text::Stretch::SemiCondensed => {
                font_kit::properties::Stretch::SEMI_CONDENSED
            }
            cosmic_text::Stretch::SemiExpanded => {
                font_kit::properties::Stretch::SEMI_EXPANDED
            }
            cosmic_text::Stretch::UltraCondensed => {
                font_kit::properties::Stretch::ULTRA_CONDENSED
            }
            cosmic_text::Stretch::UltraExpanded => {
                font_kit::properties::Stretch::ULTRA_EXPANDED
            }
        },
    }
}
...
mod tests {
    ...
    fn all_font_names_tracks_available_families() -> Result<()> {
        ...
        text_system.add_fonts(vec![Cow::Borrowed(include_bytes!(
            "../../ghost-shell-gpui/test_assets/lilex/Lilex-Regular.ttf"
        ))])?;
        ...
        text_system.add_fonts(vec![
            ...
            Cow::Borrowed(include_bytes!(
                "../../ghost-shell-gpui/test_assets/lilex/Lilex-Bold.ttf"
            )),
        ])?;
    ...
    const IBM_PLEX: &[u8] = include_bytes!(
        "../../ghost-shell-gpui/test_assets/ibm-plex/IBMPlexSans-Regular.ttf"
    );
    const LILEX: &[u8] =
        include_bytes!("../../ghost-shell-gpui/test_assets/lilex/Lilex-Regular.ttf");
    ...
    const SEPARATORS: &[char] = &[
        '\u{000a}', '\u{000d}', '\u{001c}', '\u{001d}', '\u{001e}', '\u{0085}',
        '\u{2029}',
    ];
    ...
    fn adding_fonts_invalidates_cached_line_layouts() -> Result<()> {
        ...
        let loaded_font_count = platform_text_system
            .0
            .read()
            .loaded_fonts
            .len();
