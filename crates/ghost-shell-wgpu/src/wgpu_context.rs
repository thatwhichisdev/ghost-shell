...
impl WgpuContext {
    ...
    fn new_with_options(
        ...
        let device_id_filter = match std::env::var("ZED_DEVICE_ID") {
            Ok(val) => parse_pci_id(&val)
                .context(
                    "Failed to parse device ID from `ZED_DEVICE_ID` environment variable",
                )
                .log_err(),
            Err(std::env::VarError::NotPresent) => None,
            err => {
                err.context(
                    "Failed to read value of `ZED_DEVICE_ID` environment variable",
                )
                .log_err();
                None
            }
        ...
        let (adapter, device, queue, dual_source_blending, color_texture_format) =
            ghost_shell_gpui::block_on(Self::select_adapter_and_device(
                &instance,
                device_id_filter,
                surface,
    ...
    fn select_color_texture_format(
        adapter: &wgpu::Adapter,
