...
pub fn derive_refineable(input: TokenStream) -> TokenStream {
    ...
    let refineable_attr = attrs
        .iter()
        .find(|attr| attr.path().is_ident("refineable"));
        ...
        .map(|f| {
            if derives_serialize {
                if is_refineable_field(f) {
                    quote! { #[serde(default, skip_serializing_if = "::ghost_shell_gpui::IsEmpty::is_empty")] }
                } else {
    ...
    let r#gen = quote! {
        ...
        impl #impl_generics ::ghost_shell_gpui::IsEmpty for #refinement_ident #ty_generics
            #where_clause
        {
...
fn get_wrapper_type(field: &Field, ty: &Type) -> syn::Type {
    if is_refineable_field(field) {
        ...
        let refinement_struct_name = if struct_name
            .to_string()
            .ends_with("Refinement")
        {
