...
use quote::quote;
use syn::{
    Attribute, Expr, FnArg, Ident, Item, ItemTrait, Lit, Meta, Path, ReturnType,
    TraitItem, Type, parse_macro_input, parse_quote,
...
fn generate_reflected_trait(trait_item: ItemTrait) -> TokenStream {
    ...
    for item in &trait_item.items {
        if let TraitItem::Fn(method) = item {
            ...
            let has_valid_self_receiver =
                method.sig.inputs.iter().any(
                    |arg| matches!(arg, FnArg::Receiver(r) if r.reference.is_none()),
                );
    ...
    let method_info_entries = method_infos
        .iter()
        .map(|(method_name, doc, cfg_attrs)| {
            let method_name_str = method_name.to_string();
            let wrapper_name =
                Ident::new(&format!("__wrapper_{}", method_name), method_name.span());
...
fn is_called_from_gpui_crate(_span: Span) -> bool {
    ...
    // This is a heuristic approach - we check if the current crate name is "ghost-shell-gpui"
    std::env::var("CARGO_PKG_NAME").is_ok_and(|name| name == "ghost-shell-gpui")
}
...
fn try_expand_macro(macro_item: &syn::TraitItemMacro) -> Option<Vec<TraitItem>> {
    ...
    match macro_name.as_str() {
        "ghost_shell_gpui_macros::style_helpers" | "style_helpers" => {
        ...
        | "visibility_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded =
                crate::styles::visibility_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        "ghost_shell_gpui_macros::margin_style_methods" | "margin_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded = crate::styles::margin_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        "ghost_shell_gpui_macros::padding_style_methods" | "padding_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded =
                crate::styles::padding_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        "ghost_shell_gpui_macros::position_style_methods" | "position_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded =
                crate::styles::position_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        "ghost_shell_gpui_macros::overflow_style_methods" | "overflow_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded =
                crate::styles::overflow_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        "ghost_shell_gpui_macros::cursor_style_methods" | "cursor_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded = crate::styles::cursor_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        "ghost_shell_gpui_macros::border_style_methods" | "border_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
        ...
        | "box_shadow_style_methods" => {
            let tokens = macro_item.mac.tokens.clone();
            let expanded =
                crate::styles::box_shadow_style_methods(TokenStream::from(tokens));
            parse_expanded_items(expanded)
        }
        _ => None,
    }
