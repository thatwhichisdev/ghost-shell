...
/// Derives a refinement type and the `Refineable` implementation.
...
///
/// ```compile_fail
/// # #[macro_use] extern crate ghost_shell_gpui_macros;
/// # #[macro_use] extern crate ghost_shell_gpui;
/// #[derive(AppContext)]
/// struct MyContext<'a> {
///     app: &'a mut ghost_shell_gpui::App
/// }
/// ```
...
///     #[app]
///     app: &'a mut ghost_shell_gpui::App,
///     window: &'b mut ghost_shell_gpui::Window
/// }
/// ```
///
/// ```compile_fail
...
/// `#[ghost_shell_gpui::test]` can be used to annotate test functions that run with GPUI support.
///
...
/// # Arguments
///
/// - `#[ghost_shell_gpui::test]` with no arguments runs once with the seed `0` or `SEED` env var if set.
/// - `#[ghost_shell_gpui::test(seed = 10)]` runs once with the seed `10`.
/// - `#[ghost_shell_gpui::test(seeds(10, 20, 30))]` runs three times with seeds `10`, `20`, and `30`.
/// - `#[ghost_shell_gpui::test(iterations = 5)]` runs five times, providing as seed the values in the range `0..5`.
/// - `#[ghost_shell_gpui::test(retries = 3)]` runs up to four times if it fails to try and make it pass.
/// - `#[ghost_shell_gpui::test(on_failure = "crate::test::report_failure")]` will call the specified function after the
///   tests fail so that you can write out more detail about the failure.
///
/// You can combine `iterations = ...` with `seeds(...)`:
/// - `#[ghost_shell_gpui::test(iterations = 5, seed = 10)]` is equivalent to `#[ghost_shell_gpui::test(seeds(0, 1, 2, 3, 4, 10))]`.
/// - `#[ghost_shell_gpui::test(iterations = 5, seeds(10, 20, 30)]` is equivalent to `#[ghost_shell_gpui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
/// - `#[ghost_shell_gpui::test(seeds(10, 20, 30), iterations = 5]` is equivalent to `#[ghost_shell_gpui::test(seeds(0, 1, 2, 3, 4, 10, 20, 30))]`.
///
...
/// `#[ghost_shell_gpui::bench]` annotates a Criterion benchmark that runs with GPUI support.
///
/// Use `#[ghost_shell_gpui::bench(inputs = some_iterable())]` on benchmarks that take an
...
///
/// The benchmark crate must add `criterion` and alias `ghost-shell-gpui`
/// as `gpui`, enabling its `bench-support` feature.
#[proc_macro_attribute]
pub fn bench(args: TokenStream, function: TokenStream) -> TokenStream {
    bench::bench(args, function)
}

/// A variant of `#[ghost_shell_gpui::test]` that supports property-based testing.
///
...
/// ```
/// #[ghost_shell_gpui::property_test]
...
/// some of the same GPUI-specific arguments as `#[ghost_shell_gpui::test]`. Specifically,
...
/// `Arbitrary`. Parameters to a `#[ghost_shell_gpui::property_test]`, by default, use a
...
/// ## Scheduler
///
/// Similar to `#[ghost_shell_gpui::test]`, this macro will choose random seeds for the test
...
///
/// ```ignore
/// pub fn methods::<T: TheTrait + 'static>() -> Vec<ghost_shell_gpui::inspector_reflection::FunctionReflection<T>>;
///
/// pub fn find_method::<T: TheTrait + 'static>() -> Option<ghost_shell_gpui::inspector_reflection::FunctionReflection<T>>;
/// ```
///
...
#[cfg(any(feature = "inspector", debug_assertions))]
#[proc_macro_attribute]
pub fn derive_inspector_reflection(
    _args: TokenStream,
    input: TokenStream,
) -> TokenStream {
        ...
        syn::Data::Struct(data_struct) => data_struct
            ...
            .find(|field| {
                field
                    .attrs
                    .iter()
                    .any(|attr| attr.path().is_ident(name))
            })
